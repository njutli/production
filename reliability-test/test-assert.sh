...
# 注意：不能用 $(...) 捕获断言输出，否则子 shell 中计数器不回传
# 改用临时文件，让断言在父 shell 执行（_ASSERT_COUNT/_ASSERT_FAIL 正确累加）
_OUT=$(mktemp)
trap 'rm -f "$_OUT"' EXIT

tap_plan_start "TEST-003" "basic-fail" >/dev/null
assert_eq "abc" "xyz" "eq-should-fail" > "$_OUT" 2>&1
grep -q "^not ok 1" "$_OUT" && { echo "  ok  - assert_eq FAIL 输出 not ok"; track PASS; } || { echo "  FAIL - 未输出 not ok: $(cat "$_OUT")"; track FAIL; }
grep -q "actual='abc'" "$_OUT" && { echo "  ok  - assert_eq FAIL 含 actual"; track PASS; } || { echo "  FAIL - 不含 actual"; track FAIL; }

assert_lt 10 5 "lt-should-fail" > "$_OUT" 2>&1
grep -q "^not ok 2" "$_OUT" && { echo "  ok  - assert_lt FAIL 输出 not ok"; track PASS; } || { echo "  FAIL"; track FAIL; }

assert_match "active" "degraded" "match-should-fail" > "$_OUT" 2>&1
grep -q "^not ok 3" "$_OUT" && { echo "  ok  - assert_match FAIL 输出 not ok"; track PASS; } || { echo "  FAIL"; track FAIL; }
grep -q "!~" "$_OUT" && { echo "  ok  - assert_match FAIL 含 !~"; track PASS; } || { echo "  FAIL - 不含 !~"; track FAIL; }
...
# get_ceph_health 应包含 HEALTH（用 assert_wait_match，不能用 echo 当 func）
assert_wait_match get_ceph_health "HEALTH" 10 "assert_wait_match ceph health contains HEALTH" >/dev/null 2>&1 && { echo "  ok  - assert_wait_match ceph health"; track PASS; } || { echo "  FAIL - assert_wait_match ceph health"; track FAIL; }
...
# get_tikv_leader 应包含 ceph-node（新环境 PD 名是 ceph-node1 而非 pd-1）
assert_wait_match get_tikv_leader "ceph-node" 10 "assert_wait_match tikv_leader contains ceph-node" >/dev/null 2>&1 && { echo "  ok  - assert_wait_match tikv_leader"; track PASS; } || { echo "  FAIL - assert_wait_match tikv_leader"; track FAIL; }
...
[ "$T2" -ge 3 ] && [ "$T2" -le 30 ] && { echo "  ok  - 超时耗时 ${T2}s (expected 3-30s, cephadm shell 每次 ~5s)"; track PASS; } || { echo "  FAIL - 超时耗时 ${T2}s (expected 3-30s)"; track FAIL; }
(tap_plan_end)
# ============================================================
echo ""
...
# get_osd_status 6 应返回 "up"（新环境 OSD ID 从 6 开始）
assert_wait_eq get_osd_status 6 "up" 10 "OSD 6 status = up" >/dev/null 2>&1 && { echo "  ok  - assert_wait_eq with args (get_osd_status 6 = up)"; track PASS; } || { echo "  FAIL - assert_wait_eq with args"; track FAIL; }
(tap_plan_end)
