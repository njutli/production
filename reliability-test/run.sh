...
run_case() {
    ...
    mkdir -p "$result_dir"
    local artifacts_dir="${result_dir}/artifacts"
    ...
    # 清理上次用例在 /tmp 残留的产物，避免混入本次收集
    rm -f /tmp/reliability-fio-result.json /tmp/reliability-fio-baseline.json 2>/dev/null || true
    rm -f /tmp/reliability_monitor_*.csv 2>/dev/null || true
    rm -rf /tmp/reliability-test-metrics/* 2>/dev/null || true
    ...
    if [ -n "$snap_file" ] && [ -f "$snap_file" ]; then
        ...
        cp "$snap_file" "$artifacts_dir/" 2>/dev/null || true
    ...
    # 收集 /tmp 下的测试产物到 result_dir/artifacts
    # 快照已在上面 cp 到 artifacts_dir，这里收集其余文件
    cp /tmp/reliability-fio-result.json "$artifacts_dir/" 2>/dev/null || true
    cp /tmp/reliability-fio-baseline.json "$artifacts_dir/" 2>/dev/null || true
    cp -r /tmp/reliability-test-metrics/* "$artifacts_dir/" 2>/dev/null || true
    cp /tmp/reliability_monitor_*.csv "$artifacts_dir/" 2>/dev/null || true
    local count=$(find "$artifacts_dir" -type f 2>/dev/null | wc -l)
    [ "$count" -gt 0 ] && echo "  artifacts: ${count} files → ${artifacts_dir}" || true
}
