...
echo ""
echo "[get_metric_change_time: tikv_leader contains ceph-node]"
T=$(get_metric_change_time "tikv_leader" "ceph-node")
[ "$T" != "0" ] && [ -n "$T" ] && ok "找到 tikv_leader 含 ceph-node 时间戳: $T" || fail "未找到"
