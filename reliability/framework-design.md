...
LT与分钟级故障用例不同，不经过 `run.sh` 的统一timeout/retry：自动重试会掩盖长期状态积累，普通 `all` 误触发长测也不可接受。LT在157上使用独占固定数据集和远端nohup控制器，按小时拆分fio结果；WSL断线不终止控制器，完成后用 `collect-long-term.sh` 持久化到 `/mnt/c/SunRise/test/reliability/`。

公共引擎只包含五类动作：显式ACK布局、固定负载、只读采样/容量守卫、精确PID停止、完整CRC校验。原生LT-001～003不含compact/GC/服务变更；LT-004只对冻结manifest中的精确文件执行单线程compact。每个RUN冻结脚本、配置、阈值和命令，不自动删除数据或证据。

```
prepare: 精确路径新建数据 → CRC初验 → 冻结inode/大小manifest
start:   小时fio窗口 + 60s资源/容量守卫 + 声明检查点元数据dump
drain:   写负载停止后继续采样，不人工维护
verify:  全固定文件集CRC verify-only
analyze: 小时带宽/P95/P99/停顿 + slice/对象/DB周期 + 四维结论
collect: SHA256验证后复制到唯一权威持久化目录；不清理远端资产
```

2小时只签收安全筛查；24小时且检测到至少3个增长—回收周期才允许初步判断容量收敛；72小时仅用于拟交付组合。LT-003默认前三轮覆盖全部文件，之后仅保持前半文件为热集、后半文件转冷，以区分热写积累与冷文件后台回收；两个负载阶段分别使用本阶段首窗作相对基准。容量守卫失败后的人工维护可证明可恢复，但不能把原生运行改判PASS。
