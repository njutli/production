...
CALLS=()
stop_tikv 10.20.1.150
[ "${CALLS[0]}" = "[10.20.1.150] sudo systemctl stop tikv 2>/dev/null" ] && ok "stop_tikv" || fail "stop_tikv: ${CALLS[0]}"

CALLS=()
start_tikv 10.20.1.151
[ "${CALLS[0]}" = "[10.20.1.151] sudo systemctl start tikv 2>/dev/null" ] && ok "start_tikv" || fail "start_tikv: ${CALLS[0]}"

CALLS=()
stop_pd 10.20.1.150
[ "${CALLS[0]}" = "[10.20.1.150] sudo systemctl stop pd 2>/dev/null" ] && ok "stop_pd" || fail "stop_pd: ${CALLS[0]}"

CALLS=()
start_pd 10.20.1.152
[ "${CALLS[0]}" = "[10.20.1.152] sudo systemctl start pd 2>/dev/null" ] && ok "start_pd" || fail "start_pd: ${CALLS[0]}"

CALLS=()
stop_mon 10.20.1.150
[[ "${CALLS[0]}" == *"ceph orch daemon stop mon.ceph-node1"* ]] && ok "stop_mon: hostname 映射正确" || fail "stop_mon: ${CALLS[0]}"

CALLS=()
start_mon 10.20.1.152
...
CALLS=()
start_node_storage 10.20.1.150
...
CALLS=()
crash_node_storage 10.20.1.150
...
CALLS=()
restart_node_storage 10.20.1.150
...
CALLS=()
net_partition 10.20.1.150 10.20.1.151 6800 6801 2>/dev/null
RC=$?
...
CALLS=()
net_partition 10.3.1.6 10.3.1.7 6800 6801 2>/dev/null
RC=$?
[ "$RC" = 0 ] && ok "net_partition: 允许 100GbE 网段" || fail "net_partition: 应允许 100GbE"
...
for c in "${CALLS[@]}"; do
    ...
    [[ "$c" == *"10.3.1.7"* ]] && found_dst=1
done
[ "$found_port" = 1 ] && ok "net_partition: 含 --dport 6800" || fail "net_partition: 不含端口"
[ "$found_dst" = 1 ] && ok "net_partition: 含对端 IP 10.3.1.7" || fail "net_partition: 不含对端 IP"
...
CALLS=()
net_partition 10.3.1.6 10.3.1.7 2>/dev/null
RC=$?
