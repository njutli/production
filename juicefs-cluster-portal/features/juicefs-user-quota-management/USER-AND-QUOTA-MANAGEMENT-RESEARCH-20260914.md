...
第一期所说的“普通用户不能自行提高配额”，针对的是没有sudo、Docker管理、特权容器、宿主机root或挂载服务控制权的文件用户。Portal服务、配额服务和集群运维人员属于可信管理范围。独立进程能限制误操作和权限扩散，但不能把已被攻陷的Portal自动变成可信的请求来源。

### 11.2 现有系统可复用能力及必须调整的部分

| 现状核对结果 | 本方案处理 |
|---|---|
| 152已有Go Portal、HTTPS、Argon2id密码哈希、ADMIN/USER、Prometheus和目录快照 | 复用这些组件；现有监控接口继续只读 |
| USER按`namespaceRoots`授权；部署模板把普通示例账号绑定到整个业务卷根 | 多用户开放前撤销普通账号的整卷授权，改为显式用户目录绑定；不把旧授权与新授权合并放行 |
| 账号文件只在启动时读取；现有账号工具仅支持初始化 | 增加账号新增、禁用、密码重置及安全重载；不向Portal开放任意系统账号操作 |
| Cookie无账号认证版本；旧管理员Bearer Token直接取得ADMIN角色 | 新写接口只接受有效的个人管理员会话；旧Bearer Token保持只读。增加旧会话失效机制 |
| 152的专用挂载为支持`.control`统计，技术上可写；Portal靠路径隔离看不到它 | 明确称为“专用统计控制挂载”，不把它当作后端只读权限；配额操作不依赖该挂载 |
| Portal、Prometheus/Grafana、统计挂载和collector的部署模板共享`jfsportal`，SQLite主要在连接层设为只读 | Web迁入独占账户，统计及配额另用账户；新增控制库，保持Web对统计及控制快照的文件级只读访问 |
| `summary`结果是三级top 100明细，采集存在root数量、超时和计量口径限制 | 配额页直接读取JuiceFS quota计数；私有目录明细单独验收，不拿现有`logicalBytes`字段直接计算剩余额度 |
...
| 操作者 | 可以做什么 | 使用什么身份 |
|---|---|---|
| 集群/客户端运维人员 | 分配UID/GID、创建Linux账号和目录、管理挂载与后端凭据、处理离职及数据移交 | 相关节点上获授权的root/sudo；需要网页操作时另用Portal ADMIN |
| Portal ADMIN | 登记存储用户、导入已有目录、设置/调整配额、分配目录查看权限、查看全部管理记录 | Portal个人管理账号；不自动获得服务器root |
| 普通文件用户 | 在权限允许的目录中读写，受到JuiceFS配额限制 | Linux进程的UID/GID与附加组 |
| Portal USER（可选） | 查看被明确授权目录的额度、用量和已验收的目录信息 | 独立Portal账号；不会取得TiKV/Ceph凭据 |
| 挂载、统计及配额服务 | 分别承担文件访问、统计、固定管理动作 | 独立系统服务账户及必要的后端凭据，不作为人员登录账号 |

台账按一个受管身份域记录用户名、UID、主GID、附加组及允许登录的客户端。UID不能分给两个人；同一GID可以由多名用户共享，不能给每条用户记录的GID都加唯一约束。跨客户端的同一人必须采用一致的有效数字身份；新客户端加入前同时检查本地和NSS身份源，不能只检查Portal数据库。

第一期保留“一名文件用户对应一个受管私有目录”的默认关系，但将Portal账号与文件用户分开保存。个人Portal账号默认只绑定自己的资源；共享项目或额外委托查看必须由ADMIN显式授权，不能根据用户名相同、同GID或路径前缀自动推断。停用后的UID和资源记录保留，数据未移交前不回收编号。
...
管理员/普通用户浏览器
    └─ HTTPS :8443 → 152 Portal（jfsweb，拟新增）
                       ├─ 只读 → Prometheus、目录快照、配额/资源快照
                       └─ ADMIN管理请求／查看权限校验 → Unix Socket
                                           ↓
                              152 quota-controller（jfsquota）
                                 ├─ 固定卷的JuiceFS元数据接口 → PD/TiKV 150～152
                                 └─ 控制库、操作审计、只读发布快照
...
建议新增`jfsquota`承担配额控制，将统计挂载和collector迁入`jfsnamespace`，Web迁入由Portal独占的`jfsweb`账户；Prometheus/Grafana可暂保留原账户，避免顺带重部署监控组件。独占Web账户是Unix Socket对端UID校验的前提：不能让其他同UID进程也获得配额调用权。迁移时同时调整文件权限和systemd访问范围，不仅更改unit的`User`字段。

配额服务仅需访问TiKV元数据，不配置Ceph keyring，也不访问JuiceFS文件内容；但它仍是持有高权限元数据访问能力的可信服务。

新控制状态位于152系统盘的独立目录，例如`/var/lib/juicefs-quota/`；凭据位于独立受保护配置目录。私有控制库由`jfsquota`写入，向Web发布去除凭据的只读快照。采用独立目录、文件所有权与systemd限制，避免“SQLite连接只读，但Web进程仍能改数据库文件”的情况。快照发布必须保持完整一代的一致性，不能复制正在写入的裸SQLite/WAL文件。
...
以Alice、UID/GID `10001:10001`、目录`/users/alice`、额度100 GiB为例，编号和额度均为示例，正式值由管理员分配。

1. **分配身份。** 运维人员在台账登记Alice，核对157及其他获准客户端没有用户名、UID或GID冲突。若采用本地账号，在每台允许她登录的客户端创建一致身份；若以后接入LDAP，则在目录服务创建一次并核验客户端解析结果，不再逐台重复`useradd`。
2. **创建目录但暂不开放写入。** 157上的root确认业务JuiceFS挂载及卷UUID，在管理员拥有的`/users`下创建`alice`目录，暂由root持有且设为0700。`/users`不能由普通用户写入，建议0711或按业务要求设置只允许穿越的权限。此时暂不开通Alice的业务写入。
3. **在Portal登记。** ADMIN创建“存储用户”记录，选择已登记的卷并填写逻辑目录`/users/alice`、计划owner和额度。配额服务实际读取目录类型、inode、现有owner及卷UUID后建立资源记录；新建流程允许已知的root暂存owner，不能把其他用户目录当作待开通资源。
4. **设置规则。** 配额服务通过JuiceFS原生接口设置100 GiB容量和经确认的inode限额，并回读核对。配置失败或结果不确定时保留未开放状态，不提前宣布开通完成。
5. **确认交付条件。** 等待隔离验收确定的配置传播窗口，核对使用的挂载版本与状态。界面明确区分“元数据配置已确认”和“客户端传播中”；不把一次回读当作所有客户端都已确认。
6. **交付目录。** 运维人员在157将目录owner改为`10001:10001`并保持0700，再由配额服务核对inode未变、owner符合计划，激活资源。其他客户端读取的是同一套元数据，无需在那里再创建同一JuiceFS目录。
7. **开通登录。** 完成所需Linux登录方式后，Alice从客户端使用该目录。她的文件操作沿用现有FUSE→TiKV/Ceph路径，Portal不参与数据请求。
8. **按需开通门户。** 若Alice需要查看用量，创建个人Portal USER账号并显式绑定该资源。无需Portal账号时，此步跳过；这不会影响Linux文件访问或配额执行。

已存在且有数据的用户目录采用“导入”流程：不改owner、不自动搬数据，先核对身份、目录和实际用量，再初始化配额。首次统计可能较慢，必须异步执行；要保证导入过程中不出现无配额写入窗口时，应由运维人员暂停该目录的写任务。不能将不受约束的既有目录追溯性地声明为一直受保护。

#### 11.5.2 增加客户端与修改用户信息

新客户端先完成UID/GID冲突检查，再配置获准的挂载服务及后端连接；挂载同一卷即可使用已有目录。首期普通用户不自行创建挂载、不自行修改客户端身份映射。
...
| 多项配额叠加 | P0及目录导入时核对已有卷级、祖先目录、UID/GID配额，第一期即提示其他有效限制；目录剩余额度不等于实际还能写入的唯一上限。未完成核对时明确标为未知，不宣称无其他限制 |
...
|---|---|
| `GET/POST /admin/storage-users` | ADMIN查看或登记文件用户台账；不创建Linux账号 |
| `GET/POST /admin/storage-resources` | ADMIN查看或导入已有目录，建立卷和inode绑定 |
...
第一期采用一个独立SQLite控制库及其只读发布快照，无需额外部署Redis、消息队列或数据库集群。

| 记录 | 最小字段及约束 |
|---|---|
| 存储用户/组 | 稳定ID、身份域、用户名、UID、主GID、附加组、客户端范围、状态；用户UID及组定义各自唯一，用户可共享GID |
| 卷 | 内部ID、真实JuiceFS UUID、配置引用、允许管理根、分配预算；数据库不保存可向Web暴露的秘密 |
| 目录资源 | 稳定resource ID、卷UUID、路径、inode、owner、绑定用户、状态；同卷下活动路径及inode不得重复绑定 |
| 查看授权 | Portal稳定用户ID与resource ID的显式关系；普通账号无默认整卷权限 |
| 配额观测 | 真实上限、用量、观测时间、来源版本、是否过期；与拟修改值分别保存 |
| 操作与审计 | operation ID、幂等键、操作者、目标、旧值、新值、预期版本、理由、时间、阶段、结果和回读值 |

账号密码暂继续由受保护的本地账号文件管理，补齐`portal-userctl`的新增、禁用、重置功能及原子重载，避免为本次功能再建一套密码库。账号增加稳定ID和`auth_version`，密码重置、角色变更及禁用使旧会话失效；账号删除后同名重建不能继承旧会话或目录授权。

用量快照可以短暂过期，**查看授权不能沿用过期授权继续放行**。为简化撤权一致性，Portal每次读取受保护的资源数据前，通过本机Unix Socket让控制器查询当前授权关系；这只读控制库，不访问TiKV或递归扫描。撤销提交控制库后，新请求不再放行；控制器不可用时拒绝受保护数据访问。旧快照即使仍包含该资源，也不能作为授权依据；已经交付浏览器的数据无法追溯收回。

SQLite只读查询采用一致性事务或完整代快照，避免读取root后其对应entries已被另一代替换。现有Bearer身份没有个人资源绑定时，不得继承普通示例账号的整卷查看权。

配额调整流程为：

```text
预览 → 提交并记录任务 → 核对当前真实值/版本 → 串行执行
     → 回读一致：配置已确认
     → 未调用后端即失败：失败
     → 已调用但超时/断连：结果待确认 → 重新读取真实值后归档
```

初期单worker串行处理变更，减少并发复杂度。同一幂等键重复提交同一内容返回原任务，提交不同内容返回冲突；执行时重新检查控制库版本及后端真实上限，避免Portal内两个管理员相互覆盖。这里的版本是控制库管理版本，不是JuiceFS原生quota revision/CAS；不断变化的用量也不能当作上限修改版本。

单worker不能阻止外部root CLI在检查与设置之间修改同一配额。因此受管资源的日常变更统一经控制器，紧急CLI操作须先暂停该资源的管理任务，事后重新核对；不能承诺对任意外部写入也实现原子防覆盖。对外返回“已确认”必须在审计和回读结果持久化后进行。TiKV提交与SQLite写入不能视为一个事务；服务重启后先对未完成任务进行核对，不盲目重放或自动回滚所有配额。
...
配额服务初期每60秒批量读取一次已登记资源的配额，页面每30秒刷新；修改后对目标资源立即回读并更新。普通GET请求只读快照，不同步运行CLI或递归扫描；超过180秒未成功刷新时保留旧值并标记过期，不显示为0。权限变更按11.8节独立及时生效，不等待上述用量刷新周期。
...
| 目录或配额快照缺失、损坏、schema不兼容 | 相关页面显示不可用，不阻止整个Portal启动或现有监控访问；不能核验授权时仍拒绝受保护数据读取 |
...
152新增服务先采用低并发、CPU约0.25核和内存512 MiB的资源上限作为验证起点，实测后调整；长时间初始统计单独限时排队。管理数据只写152系统盘专用目录，配置、控制库和审计定期备份；原始验收数据按项目约定保存至`/mnt/c/SunRise/test/`，凭据不进入报告或结果包。
...
| P1：本地开发 | 配额适配器、控制库、幂等worker、API/UI、账号撤销、资源授权和只读快照 | 离线功能与权限检查通过，使用模拟后端不改集群 |
...
| P4：小范围开通 | 先导入一两个用户目录，再完成一次增额、一次受控降额、一次用户停用 | 操作审计、两客户端生效、管理面故障不影响数据面均通过，再扩大用户范围 |

最小验收覆盖下列问题，每项保留必要证据即可：

1. **身份与私有目录**：Alice在两台客户端使用同一UID/GID；Bob不能访问她的0700目录；UID冲突的新客户端被开通流程拒绝。
2. **网页权限**：USER不能访问他人resource ID、整卷根、管理接口；匿名、旧Bearer和伪造请求不能设置配额；密码重置/禁用后旧Cookie失效。授权撤销后即使保留旧数据快照、发生控制器重启，也不能继续放行新请求。
