# JuiceFS 集群管理门户开发与部署说明

> 当前部署：`10.20.1.152（ceph-node3）`
>
> 适用范围：后续功能开发、静态资源或服务更新、健康验收与回滚
>
> 三个大阶段的安排见[总体开发计划](../DEVELOPMENT-ROADMAP.md)。本文记录现有环境的开发与更新操作；第三阶段面向客户现场的[快速部署方案](../stages/03-deployment/CUSTOMER-DEPLOYMENT-DESIGN-20260922.md)及[开发计划](../stages/03-deployment/CUSTOMER-DEPLOYMENT-DEVELOPMENT-PLAN-20260922.md)另行维护。
>
> 安全原则：所有远端写操作遵守仓库 `skills/SYSTEM-SAFETY-SKILL.md`；sudo 命令必须逐项另行审批

## 1. 代码和配置

下表路径相对于本文；后续命令均在Portal根目录执行。阶段文档按开发阶段归档，代码与配置仍为全平台共用。
...
| `../tests/` | 总离线 Gate |
| `../stages/01-readonly-monitoring/docs/` | 当前有效范围、指标、数据模型和目录快照合同 |
| `../stages/01-readonly-monitoring/inventory/` | 当前部署的最终签收证据 |
| `../stages/02-user-quota/` | 用户与配额管理的调研、方案和开发计划 |
| `../stages/03-deployment/` | 客户现场快速部署方案及开发计划 |
...
详细合同见[目录快照设计](../stages/01-readonly-monitoring/docs/T09-DIRECTORY-SNAPSHOT-DESIGN.md)。

## 7. 回滚和文档

任何回滚都必须使用对应RUN备份中的精确脚本，并重新提交sudo审批；禁止通配删除、递归改属主或从不明备份恢复。回滚后重复完整只读验收。

当前功能与访问方式见[第一阶段功能说明](../stages/01-readonly-monitoring/CURRENT-STAGE-SYSTEM-OVERVIEW-20260914.md)；部署裁决见[验收摘要](../stages/01-readonly-monitoring/inventory/CURRENT-DEPLOYMENT-ACCEPTANCE-SUMMARY-20260914.md)；未完成事项见[TODO](../TODO.md)。
