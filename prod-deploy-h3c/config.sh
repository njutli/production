...
JUICEFS_FS_NAME="juicefs-h3c"       # H3C 口径专用卷（block-size 4M，独立于 juicefs-prod 的 256K 卷）
JUICEFS_MOUNT_POINT="/mnt/juicefs"  # prod-deploy 口径挂载点
EPC_MOUNT_POINT="/mnt/epc"          # H3C 对比口径挂载点（h3c 测试用）
...
# H3C 口径 = 大块顺序单线程，与 prod-deploy 的小块随机高并发相反：
#   - block-size 4M：顺序读无半块放大；RADOS GET/PUT 数 16×↓（需 format 时一致，见 JUICEFS_FORMAT_BLOCK_SIZE）
#   - max-fuse-io 1M：kernel 5.15 FUSE 硬上限；mseqwrite 随 dispatch 数减少单调涨（02-1 §9.2：2265→4279，+89%）
#   - buffer-size 1024：配合 max-fuse-io 1M，防 go-fuse readPool 涨内存触发 write sleep（02-1b）
JUICEFS_BASE_MOUNT_OPTS=(
    --storage ceph                     # 08_1：直连 RADOS，随机写 +71%
    --bucket ceph://juicefs-data
    --block-size 4M                    # H3C 口径：顺序大块，GET/PUT 数 16×↓（与 format 一致）
    --max-fuse-io 1M                   # 02-1 §9.2：大块顺序 dispatch 数 ↓，mseqwrite +89%
    --buffer-size 1024                 # 02-1b：配合 1M 防 write sleep
    --max-uploads 150                  # 演进报告 §四：顺序写 +23%
)

# format 时的卷级参数（block-size 必须与 mount 一致，否则以 format 值为准）
JUICEFS_FORMAT_BLOCK_SIZE="4M"

JUICEFS_ENABLE_WRITEBACK=false       # 突发写负载 + 缓存盘空间充足时置 true；冷态测试先不开
# H3C 对比口径：全顺序读，预读是纯收益（prod-deploy 的 ra0 对 seqread -33%，方向相反）。
# prod-deploy 用 "default"(~2M) 或 "0"（关预读，消随机读放大）。
# ⚑ 8M = 2 个 block-size(4M) ahead 是经验外推：prod-deploy 从未实测过 >0 的 readahead 值，
#    此值需在 01 基线阶段 sweep 验证（候选 default/4M/8M/16M），不视为已验证最优。
JUICEFS_READAHEAD="8M"
...
[ "${JUICEFS_READAHEAD}" != "default" ] && _jfs_cache_args+=( --max-readahead "${JUICEFS_READAHEAD}" )
