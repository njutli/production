...
>
> 当前一句话结论：最后完成验证的 frozen main 仍存在目标漏派发行为，B 能纠正该行为；真实 v1.3 Ceph randwrite 性能尚未完成新一轮 S/A/B 验证，社区提交证据链正在 C03-R5 做最后收口。
...
| C03 | patch/test 是否能在执行时 main 上洁净回放，并形成社区提交材料 | 技术结论已收敛，证据审计由 R5 最终收口 |
...
| main 社区本地证据收口 | R5 最后一轮 | GLM 约 3～6 小时，Codex 复核约 1 小时 |
| issue/PR 最终材料 | 草稿已有，待最终代码和重复性审阅 | 约 1～2 小时 |
| v1.3 port、correctness、Ceph build | 本地技术证据已有 | 0 |
| v1.3 远端 ABI/read smoke | 未完成 | 约 0.5～1 小时 |
| v1.3 完整 S/A/B 性能矩阵 | pool objects 安全门阻塞 | 获得安全窗口后约 8～12 小时 |
| 社区 review/merge | 尚未提交 | 外部时间，无法承诺 |

### “结束”的三个口径

1. **C03/社区提交准备结束**：R5 得出 PASS、UPSTREAM-ALREADY-FIXED、技术失败或规定的 BLOCKED 终态，并经 Codex 复核后结束。目标是在 2026-08-18 当天收口。
...
| 上游竞态修复提交 | 不存在；二分找到的分界 = #6336（max-fuse-io 旗标失效修复） |
