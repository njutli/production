...
> ## ⛔ 状态：暂缓（SUPERSEDED），⛔ 不要执行本任务书
>
> **取代者：`doc/perf-tasks/u141d-randwrite-randrw-confirm-10pct.md`（8 轮 / 单夜 / M_eng=10%）。**
>
> 暂缓理由（2026-08-29 决策）：本任务书为「闭合 7 项 @ `M_eng`=3%」而设计，需 3–5 夜。
> 但 3% 这个阈值从 U141 起被逐份继承、**从未按业务口径论证过**，而每轮固有噪声实测为 1.0–3.4%
> ⇒ 3% 的要求比测量精度还细，需 16–20 轮，这是算术约束而非流程问题。
> 复核显示：把阈值按业务定为 **7.5–10%** 后，U141b 已有的 8 轮数据即可闭合 6/6 已测项
> （见现状报告与 §2.7 功效表）⇒ 正确做法是**降低问题的精度要求**，而不是加大取样。
>
> ⚑ 本文件**保留不删**，其中三部分仍是有效资产，U141d 已直接复用：
> ① §2.4 点估计量锁定（mean）与 D36 静态检查；
> ② §2.5 回归式判据（`arm` 系数 + 单侧 CI，取代 `M=2ε`）；
> ③ §2.7 功效计算表（由 U141b 实测残差尺度推出，是"还差多少轮"的唯一依据）。
>
> **仅当业务明确需要 3% 分辨力时才重启本任务书**；重启前须先按 §2.6 冻结 `M_eng`。
