# 任务书 00：新集群全量冷态基线测试（default vs ra0 双组对照）【上一阶段，已归档】

> 归档说明：本篇为**上一阶段**的双组（default/ra0）× 双口径（不限速/千兆）全量基线任务书，已执行完成。
> 当前"不限速调优"阶段从 `01-1-cluster-deploy-and-nolimit-baseline.md` 起（含集群从零部署），
> 且已决定**停做千兆限速**（依据 `doc/perf-analysis/01-baseline-review-and-nolimit-plan.md` §四）。
