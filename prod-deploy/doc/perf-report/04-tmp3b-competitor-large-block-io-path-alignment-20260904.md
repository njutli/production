...
## 九、04-tmp3c 后续订正（2026-09-04）

04-tmp3c 在同一个 B4 卷内完成 RA8/RA32 ABBA：RA32 相对相邻 RA8 分别提升 `73.60%/77.55%`，
在途 GET 从约 `2.33` 提升到 `4.54--4.59`；两侧 B256/RA8 锚漂移仅 `-1.56%`。因此本报告
“B4+RA8 较慢”的原始事实仍有效，但“足够否决 4 MiB BlockSize”以及“最终保持 RA8”的推论已被
推翻。正确窄结论是：固定 RA8 对 B4 构成对象并发限制，B4/RA32 成为 L2 候选；是否生产可用仍需
04-tmp3d 对象层屋顶与随机/写/七项回归。详见
`04-tmp3c-blocksize-readahead-concurrency-decoupling-20260904.md`。
