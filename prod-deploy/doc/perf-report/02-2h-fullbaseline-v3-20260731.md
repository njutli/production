...
seqwrite r3=1335 vs 其余 1552~1598，偏差 14.6%。不是冷启动（r1 正常），疑似 compaction 时序问题（r2 后 compact 15s，可能 r3 启动时 LSM tree 仍有残留）。r4 恢复到 1552。

### 4.2 128-job spec（randrw-128）

> 确定性预热 #2 后运行。128 job × 1G，bs=256k，libaio，iodepth=128，randrw 50/50。

| 轮次 | fio 聚合 BW (MiB/s) | 稳态中位数 (MiB/s) |
|------|---------------------|-------------------|
| r1 | 1228 | 1347 |
| r2 | 1320 | — |
| r3 | 1193 | — |
...
> 注：稳态评估只输出了整体 range/median/CV，未输出 per-round 稳态值。r1 和 r5 的稳态值（1347, 911）来自 range 端点。

**单调下降趋势**：r1=1347 → r5=911（-32%），5 轮持续下降，非随机波动。可能原因：
...
**结论**：128-job 高并发随机读写的工作集超过缓存容量时，BlueStore 缓存被逐步驱逐，BW 单调下降。单进程因工作集小（~7GB/轮）不受此影响。128-job 的稳定性需用交错 A/B 法（每轮重新预热）。
...
| 用途 | 适用性 | 说明 |
|------|--------|------|
| 单进程 randread 基线 | ✅ 可用 | 最大偏差 0.6%，极稳 |
| 单进程 mseqread/mseqwrite 基线 | ✅ 可用 | 最大偏差 <1.5% |
| 单进程 randwrite 基线 | ✅ 可用 | 最大偏差 4.9% |
| 单进程 randrw 基线 | ⚠️ 勉强 | 最大偏差 5.3% |
| 128-job randrw 基线 | ❌ 不适用 | 单调下降 25%，需交错 A/B（每轮重新预热） |
| 跨轮绝对值比较 | 待验证 | 需第二轮数据 |
...
1. **128-job randrw 无法多轮稳定**：10 轮测试 BW 从 1199 降到 474-605，最大偏差 77%，无收敛迹象。BlueStore 缓存被逐轮驱逐，且 aggressive_cleanup（compact + drop_caches）无法恢复 BlueStore 内部缓存状态。

2. **单轮 + 预热是 128-job 唯一可靠方式**：预热后跑 1 轮（r1=1199/稳态 1204），缓存状态最优时取值。多轮测试会因缓存驱逐而持续退化。

3. **对比单进程**：单进程 randread 5 轮偏差 0.6%（每轮 ~7GB << 缓存 15GB，不驱逐）；128-job 每轮 ~200GB >> 缓存 15GB，彻底驱逐。工作集与缓存大小的比值决定稳定性。

4. **生产含义**：持续高并发随机读写负载下，系统性能会从 ~1200 MiB/s 逐渐退化到 ~500-600 MiB/s。这不是测试波动，是缓存被持续驱逐后的真实退化。
