...
RESULTS_DIR="/tmp/opencode-baseline-256k-buf1024-default"
FSID="4f4e3ca0-8297-11f1-a671-97520597268c"
NIC_IF="enp139s0f0np0"
# max-fuse-io 256K + buffer-size 1024 + ra0
MOUNT_OPTS="--max-uploads 150 --cache-size 0 --max-fuse-io 256K --buffer-size 1024"
...
clean_volume() {
    ...
    sleep 3
    mount | grep juice | grep -q max_read=262144 || { log "ERROR: max_read not 256K!"; exit 1; }
    mkdir -p "${TEST_DIR}"
    log "清卷 complete (max_read=256K, default readahead, buffer-size=1024)"
}
...
remount_jfs() {
    ...
    sleep 3
    mount | grep juice | grep -q max_read=262144 || { log "ERROR: max_read not 128K!"; exit 1; }
    log "Remounted OK (max_read=256K, default readahead, buffer-size=1024)"
}
...
mount | grep juice | grep -q max_read=262144 || { log "ERROR: max_read not 128K!"; exit 1; }
mkdir -p "${TEST_DIR}"
log "=== Pre-test setup complete (max_read=256K, default readahead, buffer-size=1024) ==="
...
log "mount: max_read=256K (--max-fuse-io 256K), ra0 (--max-readahead 0), buffer-size=1024"
