...
BW_LOG_DIR="/tmp/jfs-bw-mon-128k"
RESULTS_DIR="/tmp/opencode-monitor-test-128k"
...
juicefs mount -d ${JUICEFS_MOUNT_OPTS} "${META}" "${MNT}" 2>&1 | tail -2
sleep 3
mount | grep juice | grep -q max_read=131072 && log "Remounted OK for randwrite" || { echo "ERROR"; exit 1; }
