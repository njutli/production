...
# FULLBASELINE_V3.sh — JuiceFS/Ceph 基线测试 V3（确定性预热 + 单进程稳定基线 + 128-job spec 验证）
#
# V2：锁定 layout + 覆盖写 → 同轮 CV<5%，但跨轮 ±12%（BlueStore 缓存残留）
# V3 改进：
#   1. 确定性预热（顺序读 128G layout，填充 onode 缓存到已知状态，消除 r1 冷启动）
#   2. randread/randwrite/randrw 改为单进程（消除并发随机预热的不可控性）
#   3. 末尾加 128-job randrw（原始规格测试），前面加确定性预热
#
# 测试顺序：
#   [确定性预热] ← 填充 onode 缓存
#   seqread(1) → mseqread(16) → randread(1) → randrw(1) → seqwrite(1) → mseqwrite(16) → randwrite(1)
#   [确定性预热] ← 重置缓存为已知态
#   randrw(128) × REPEAT（规格测试）
#
# 用法（在 157 上运行）：
#   bash FULLBASELINE_V3.sh dry-run              # 纯检查
#   bash FULLBASELINE_V3.sh A 180 5 --layout    # 第一次：layout + 全量测试
#   bash FULLBASELINE_V3.sh B 180 5              # 复用 layout + 全量测试
#   bash FULLBASELINE_V3.sh C 180 5 --remount   # 改参数 + 全量测试
#
...
# 确定性预热：顺序读全部 layout 数据，让 BlueStore 缓存进入已知状态
deterministic_warmup() {
    log "=== 确定性预热（顺序读 128G layout，填充缓存到已知状态）==="
    local count=0
...
compute_hitrate_delta() {
    local outdir="$1"
    python3 - "${outdir}/hit-rate.txt" <<'PYEOF' 2>/dev/null || true
import sys
...
if not pre or not post:
    print("hit_rate: N/A (pre/post missing)")
    sys.exit(0)
def parse(l):
    parts = l.split()
    d = {}
...
pre_d = parse(pre)
post_d = parse(post)
dh = post_d["hit_bytes"] - pre_d["hit_bytes"]
dm = post_d["miss_bytes"] - pre_d["miss_bytes"]
doh = post_d["onode_hits"] - pre_d["onode_hits"]
dom = post_d["onode_misses"] - pre_d["onode_misses"]
hit_pct = dh / (dh + dm) * 100 if (dh + dm) > 0 else 0
onode_pct = doh / (doh + dom) * 100 if (doh + dom) > 0 else 0
print(f"delta: hit_bytes={dh} miss_bytes={dm} onode_hits={doh} onode_misses={dom}")
print(f"hit_rate={hit_pct:.2f}% onode_hit_rate={onode_pct:.2f}%")
PYEOF
}
...
run_fio() {
    ...
    # hit% 采集 POST + 计算 delta
    collect_hitrate "post" "${subdir}"
    local hitrate_line
    hitrate_line=$(compute_hitrate_delta "${subdir}" 2>/dev/null | tr '\n' ' ' || echo "N/A")
    log "  ${label} hit%: ${hitrate_line}"
    # jfs-stats POST
    if [ -n "${jfs_pid:-}" ]; then
...
item_randwrite() {
    ...
    for r in $(seq 1 "${REPEAT}"); do
        run_fio "randwrite-${LABEL}-r${r}" --directory="${TEST_DIR}" --name=storage_test \
            ...
            --bs=256k --rw=randwrite --ioengine=libaio --iodepth=128 --numjobs=${TEST_JOBS} \
            --direct=1 --fallocate=none --openfiles=${TEST_JOBS} \
...
item_randread() {
    ...
    for r in $(seq 1 "${REPEAT}"); do
        run_fio "randread-${LABEL}-r${r}" --directory="${TEST_DIR}" --name=storage_test \
            --filesize=${LAYOUT_SIZE} --size=${LAYOUT_SIZE} \
            --bs=256k --rw=randread --ioengine=libaio --iodepth=128 --numjobs=${TEST_JOBS} \
            --direct=1 --fallocate=none --openfiles=${TEST_JOBS} \
            --group_reporting --time_based --runtime=${RUNTIME}
    done
}
...
item_randrw_single() {
    # 单进程混合读写 + aggressive_cleanup
    for r in $(seq 1 "${REPEAT}"); do
        run_fio "randrw-single-${LABEL}-r${r}" --directory="${TEST_DIR}" --name=storage_test \
            --filesize=${LAYOUT_SIZE} --size=${LAYOUT_SIZE} \
            --bs=256k --rw=randrw --ioengine=libaio --iodepth=128 --numjobs=${TEST_JOBS} \
            --direct=1 --fallocate=none --openfiles=${TEST_JOBS} \
            --group_reporting --time_based --runtime=${RUNTIME}
        aggressive_cleanup
    done
...
item_randrw_128() {
    # 128-job 规格测试（原始口径，用于对外汇报）
    for r in $(seq 1 "${REPEAT}"); do
        run_fio "randrw-128-${LABEL}-r${r}" --directory="${TEST_DIR}" --name=storage_test \
            --filesize=${LAYOUT_SIZE} --size=${LAYOUT_SIZE} \
...
summary() {
    ...
    for item in seqread mseqread randread randrw-single seqwrite mseqwrite randwrite randrw-128; do
        local line="  ${item}:"
        for r in $(seq 1 "${REPEAT}"); do
            local bw
...
item_cfg = {
    ...
    "randrw-single":  (0, read_trim),
    "randrw-128":     (0, read_trim),
...
items = ["seqread", "mseqread", "randread", "randrw-single", "seqwrite", "mseqwrite", "randwrite", "randrw-128"]
for item in items:
...
case "${1:-}" in
    ...
    "")
        ...
        echo "  --allow-restart  允许在 OSD restart 后运行（跳过 up_from 守卫）"
        echo ""
...
log "============================================"
log "=== FULLBASELINE V3 label=${LABEL} runtime=${RUNTIME}s repeat=${REPEAT} ==="
log "=== 确定性预热 + 单进程稳定基线 + 128-job spec ==="
...
# 确定性预热（填充 onode 缓存到已知状态，消除 r1 冷启动）
deterministic_warmup

# 单进程基线测试（读在前写在后）
log "=== ${LABEL}: seqread (1 job) ==="; item_seqread
log "=== ${LABEL}: mseqread (16 jobs) ==="; item_mseqread
log "=== ${LABEL}: randread (1 job) ==="; item_randread
log "=== ${LABEL}: randrw (1 job) ==="; item_randrw_single
log "=== ${LABEL}: seqwrite (1 job) ==="; item_seqwrite
log "=== ${LABEL}: mseqwrite (16 jobs) ==="; item_mseqwrite
log "=== ${LABEL}: randwrite (1 job) ==="; item_randwrite

# 确定性预热 + 128-job 规格测试
deterministic_warmup
log "=== ${LABEL}: randrw (128 jobs spec) ==="; item_randrw_128
