...
[[ "$INSTANCE" == SEED-CANARY || "$INSTANCE" == SEED-FORMAL ]] ||
  t64_die 'seed script accepts only SEED-CANARY or SEED-FORMAL'
[[ "$CLUSTER" == A ]] || t64_die 'seed creation is frozen to cluster A'
...
t64_require_tools "$T64_JUICEFS_BIN" fio curl python3 sha256sum mountpoint truncate sync
...
verify_mount() {
  [[ -s "$STATE" ]] || t64_die 'seed volume state missing'
  mountpoint -q "$MNT" || t64_die 'seed mount is not active'
  ...
  [[ -r "/proc/$pid/stat" && $(awk '{print $22}' "/proc/$pid/stat") == "$start" ]] || t64_die 'seed mount PID changed'
  [[ $(md5sum "/proc/$pid/exe" | awk '{print $1}') == "$md5" ]] || t64_die 'seed mount binary changed'
  pairs=$(mount_pid_pairs); selected=$(t64_select_child_pid "$pairs")
  [[ "$selected" == "$pid" ]] || t64_die 'seed mount worker changed'
  t64_nul_file_has_exact_arg "/proc/$pid/cmdline" "$META" || t64_die 'seed mount META changed'
  t64_nul_file_has_exact_arg "/proc/$pid/cmdline" "$MNT" || t64_die 'seed mount path changed'
  verify_cluster
}

format_mount() {
  [[ ${T64_SEED_FORMAT_AUTH:-} == "03-22-seed-format-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t64_die "set exact T64_SEED_FORMAT_AUTH=03-22-seed-format-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  [[ ! -e "$STATE" && ! -e "$SEED_DIR/SEED_BUNDLE_PASS" && ! -e "$MNT" ]] ||
    t64_die 'seed state, bundle, or mount path already exists'
  [[ $(md5sum "$T64_JUICEFS_BIN" | awk '{print $1}') == "$T64_JUICEFS_MD5" ]] || t64_die 'JuiceFS binary MD5 mismatch'
  verify_cluster
  umask 077
  mkdir -p "$OUT" "$SEED_DIR" "$MNT"
  printf '#!/usr/bin/env bash\n# Recorded 03-22 seed commands; no password.\n' > "$OUT/commands.sh"
...
print("%s\t%s\t%s"%(s["objects"],s["stored"],s["bytes_used"]))' > "$SEED_DIR/pool-pre-format.tsv"
  if "$T64_JUICEFS_BIN" status "$META" >/dev/null 2>&1; then t64_die "seed META already exists: $META"; fi
  ...
  sleep 5
  mountpoint -q "$MNT" || t64_die 'seed mount failed'
  local pairs pid identity uuid name
  pairs=$(mount_pid_pairs); pid=$(t64_select_child_pid "$pairs")
  "$T64_JUICEFS_BIN" status "$META" > "$OUT/status-format.json"
  identity=$(t64_status_identity "$OUT/status-format.json") || t64_die 'invalid seed identity after format'
  IFS=$'\t' read -r uuid name <<< "$identity"
  [[ "$name" == "$VOLUME" ]] || t64_die 'seed volume name mismatch'
...
layout_seed() {
  verify_mount
  [[ ! -e "$OUT/LAYOUT_PASS" && ! -e "$SOURCE_DIR" ]] || t64_die 'seed layout already exists'
  mkdir -p "$SOURCE_DIR" "$OUT/jobfiles" "$OUT/layout-bw"
  if [[ "$FLAVOR" == formal ]]; then
    bash "$SCRIPT_DIR/t64-gen-jobfiles.sh" "$OUT/jobfiles" "$SOURCE_DIR" > "$OUT/jobfiles.log"
    ...
    grep -q 'err= 0' "$OUT/layout-fio.stdout" || t64_die 'formal seed layout fio lacks err=0'
    mapfile -t paths < <(awk -F= '$1=="filename"{print substr($0,index($0,"=")+1)}' "$OUT/jobfiles/layout-B0.fio" | sort -u)
    (( ${#paths[@]} == 256 )) || t64_die 'formal seed jobfile does not contain 256 unique files'
    for path in "${paths[@]}"; do
      [[ "$path" == "$SOURCE_DIR"/* && -f "$path" && ! -L "$path" ]] || t64_die "unsafe seed layout path: $path"
    ...
    grep -q 'err= 0' "$OUT/layout-fio.stdout" || t64_die 'canary seed layout fio lacks err=0'
  ...
  [[ $(wc -l < "$OUT/seed-layout-relative.tsv") -eq "$expected" ]] || t64_die 'seed layout file count mismatch'
  awk -F '\t' -v s="$size" '$2!=s{bad=1} END{exit bad}' "$OUT/seed-layout-relative.tsv" || t64_die 'seed layout size mismatch'
...
umount_seed() {
  verify_mount
  [[ -s "$OUT/LAYOUT_PASS" ]] || t64_die 'seed layout gate missing'
  record_cmd "$T64_JUICEFS_BIN" umount "$MNT"
  "$T64_JUICEFS_BIN" umount "$MNT" > "$OUT/umount.log" 2>&1 || t64_die 'graceful seed umount failed'
  mountpoint -q "$MNT" && t64_die 'seed mount remains after umount'
...
dump_seed() {
  [[ ${T64_SEED_DUMP_AUTH:-} == "03-22-seed-dump-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t64_die "set exact T64_SEED_DUMP_AUTH=03-22-seed-dump-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  [[ -s "$STATE" && -s "$OUT/LAYOUT_PASS" && -s "$OUT/READY_FOR_FIO" && -s "$OUT/UMOUNT_EPOCH" ]] ||
    t64_die 'seed layout/reset/umount evidence incomplete'
  [[ ! -e "$DUMP" && ! -e "$SEED_DIR/SEED_BUNDLE_PASS" ]] || t64_die 'seed bundle already exists'
  ! mountpoint -q "$MNT" || t64_die 'seed is still mounted'
  local then now identity uuid name
  then=$(<"$OUT/UMOUNT_EPOCH"); now=$(date +%s)
  (( now - then >= 65 )) || t64_die "seed session TTL not elapsed; retry after $((65-now+then)) seconds"
  export CEPH_CONF="$PRIVATE_CONF"
  "$T64_JUICEFS_BIN" status "$META" > "$OUT/status-pre-dump.json"
  identity=$(t64_status_identity "$OUT/status-pre-dump.json") || t64_die 'invalid seed identity before dump'
  IFS=$'\t' read -r uuid name <<< "$identity"
  [[ "$uuid" == "$(awk -F '\t' '$1=="uuid"{print $2}' "$STATE")" && "$name" == "$VOLUME" ]] || t64_die 'seed UUID/name changed before dump'
  t64_status_has_zero_sessions "$OUT/status-pre-dump.json" || t64_die 'seed has a live session; refuse dump'
  umask 077
  record_cmd env "CEPH_CONF=$PRIVATE_CONF" "$T64_JUICEFS_BIN" dump --keep-secret-key "$META" "$DUMP"
  "$T64_JUICEFS_BIN" dump --keep-secret-key "$META" "$DUMP" > "$OUT/dump.stdout" 2> "$OUT/dump.stderr"
  [[ -s "$DUMP" && $(stat -c '%a' "$DUMP") == 600 ]] || t64_die 'seed dump missing or not mode 600'
  ...
  JFS_GC_SKIPPEDTIME=0 "$T64_JUICEFS_BIN" gc "$META" > "$SEED_DIR/gc-baseline.log" 2>&1
  t64_gc_summary "$SEED_DIR/gc-baseline.log" > "$SEED_DIR/gc-baseline.tsv"
  [[ $(awk -F '\t' '$1=="leaked"{print $2}' "$SEED_DIR/gc-baseline.tsv") == 0 ]] || t64_die 'seed has leaked objects'
  [[ $(awk -F '\t' '$1=="pending"{print $2}' "$SEED_DIR/gc-baseline.tsv") == 0 ]] || t64_die 'seed has pending-delete objects'
  [[ $(awk -F '\t' '$1=="skipped"{print $2}' "$SEED_DIR/gc-baseline.tsv") == 0 ]] || t64_die 'seed GC baseline skipped objects'
  sudo ceph df --format=json | python3 -c '
import json,sys
d=json.load(sys.stdin); p=next(x for x in d["pools"] if x["name"]=="juicefs-data"); s=p["stats"]
print("%s\t%s\t%s"%(s["objects"],s["stored"],s["bytes_used"]))' > "$SEED_DIR/pool-seed.tsv"
  printf 'flavor\t%s\nvolume_name\t%s\nuuid\t%s\nsource_dir\t/seed_layout\ndump\t%s\ndump_sha256\t%s\nlayout_sha256\t%s\nanchor_sha256\t%s\njuicefs_md5\t%s\n' \
    ...
    "$(sha256sum "$SEED_DIR/seed-content-anchors.tsv" | awk '{print $1}')" "$T64_JUICEFS_MD5" > "$SEED_DIR/seed.tsv"
...
case "$ACTION" in
  ...
  umount) umount_seed;;
  dump) dump_seed;;
  *) t64_die 'usage: t64-seed-volume.sh format-mount|layout|verify|umount|dump RUN_ID A SEED-CANARY|SEED-FORMAL';;
esac
