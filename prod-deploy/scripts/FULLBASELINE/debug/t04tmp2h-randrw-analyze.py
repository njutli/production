#!/usr/bin/env python3
"""Fail-closed offline analyzer for 04-tmp2h randrw evidence."""
import argparse
import csv
import json
import math
...
CORE = ("A0-pre", "T32-R", "T32-W", "T32-P50", "T128-W", "T128-R",
        "T128-P50", "T64-R", "T64-W", "T64-P50", "A0-mid", "T256-W",
        "T256-R", "T256-P50", "T96-R", "T96-W", "T96-P50", "A0-post")
JOBS = 128
FORMAL_START, FORMAL_STOP = 15, 175
GROUP_ORDER = ("T32", "T128", "T64", "T256", "T96")
GROUP_STAGE = {"T32": ("A0-pre", "A0-mid", 1 / 4),
               "T128": ("A0-pre", "A0-mid", 2 / 4),
               "T64": ("A0-pre", "A0-mid", 3 / 4),
               "T256": ("A0-mid", "A0-post", 1 / 3),
               "T96": ("A0-mid", "A0-post", 2 / 3)}
...
def self_test(root):
    ...
    if bandwidth_window({x: 100.0 for x in range(180)})["W4_W1"] != 1: raise EvidenceError("window fixture failed")
    ramp = {x: 100.0 + .1 * x for x in range(240)}
    base_window = bandwidth_window(ramp)
    minus_one = bandwidth_window(ramp, 14, 174)
    one_second = bandwidth_window(ramp, 16, 176)
    fifty_eight = bandwidth_window(ramp, 73, 233)
    if max(abs(sample["mean_MiBs"] / base_window["mean_MiBs"] - 1) for sample in (minus_one, one_second)) >= .01:
        raise EvidenceError("+/-1s window fixture exceeded 1%")
    if max(abs(fifty_eight[key] / base_window[key] - 1) for key in ("W1_MiBs", "W2_MiBs", "W3_MiBs", "W4_MiBs")) <= .01:
        raise EvidenceError("+58s window fixture did not change W1-W4")
    for name in CORE: _fixture_cell(root / "cells" / name, 100 + (10 if name.startswith("T") and name.endswith("R") else 0), 100 + (5 if name.startswith("T") and name.endswith("W") else 0))
    if analyze(root)["RUN_VALIDITY_STATE"] != "VALID": raise EvidenceError("complete fixture failed")
    _fixture_cell(root / "cells" / "T32-P25", 130, 130); _fixture_cell(root / "cells" / "T32-P75", 140, 140)
    supplemented = analyze(root)
    if supplemented["RUN_VALIDITY_STATE"] != "VALID" or len(supplemented["groups"]["T32"]["points"]) != 5:
        raise EvidenceError("complete supplement fixture failed")
    shutil.rmtree(root / "cells" / "T32-P75")
    if analyze(root)["RUN_VALIDITY_STATE"] != "EVIDENCE_INVALID": raise EvidenceError("single supplement fixture accepted")
    _fixture_cell(root / "cells" / "T32-P75", 140, 140); shutil.rmtree(root / "cells" / "T32-P25"); shutil.rmtree(root / "cells" / "T32-P75")
    (root / "cells" / "T32-W" / "drain.tsv").unlink()
    if analyze(root)["groups"]["T32"]["points"][1]["lifecycle_safe"]: raise EvidenceError("writeback without drain was safe")
    _fixture_cell(root / "cells" / "T32-W", 100, 105)
    (root / "cells" / "T32-R" / "bw" / "randrw_bw.128.log").unlink()
    if analyze(root)["RUN_VALIDITY_STATE"] != "EVIDENCE_INVALID": raise EvidenceError("missing-job fixture accepted")
    _fixture_cell(root / "cells" / "T32-R", 110, 105)
    (root / "cells" / "T32-W" / "bw" / "randrw_bw.1.log").write_text("1000,102400,0,0\n")
    if analyze(root)["RUN_VALIDITY_STATE"] != "EVIDENCE_INVALID": raise EvidenceError("missing-direction fixture accepted")
    _fixture_cell(root / "cells" / "T32-W", 110, 105); (root / "cells" / "T32-W" / "fio-start-epoch-ns.txt").write_text("59000000000\n")
    if analyze(root)["RUN_VALIDITY_STATE"] != "EVIDENCE_INVALID": raise EvidenceError("+58s fixture accepted")
    _fixture_cell(root / "cells" / "T32-W", 110, 105); (root / "cells" / "T32-W" / "fio-start-epoch-ns.txt").write_text("2000000000\n")
    if analyze(root)["RUN_VALIDITY_STATE"] == "EVIDENCE_INVALID": raise EvidenceError("+1s fixture rejected")
    for name in CORE:
        if name.startswith("T"):
            high = name.endswith("-R")
            _fixture_cell(root / "cells" / name, 250 if high else 150, 250 if high else 150)
    if analyze(root)["adaptive_matrix"]["global"] != "EARLY_STOP": raise EvidenceError("early-stop fixture failed")
    _fixture_cell(root / "cells" / "T32-R", 105, 105)
    pressure_flip = analyze(root)
    if pressure_flip["groups"]["T32"]["pressure_robust"]: raise EvidenceError("pressure-flip fixture accepted")
    _fixture_cell(root / "cells" / "T32-R", 250, 250)
    _fixture_cell(root / "cells" / "A0-post", 120, 120)
    if analyze(root)["RUN_VALIDITY_STATE"] != "RESOLUTION_INSUFFICIENT": raise EvidenceError("anchor drift fixture accepted")
    partial = root.parent / "partial-fixture"
    for name in CORE[:CORE.index("A0-mid") + 1]:
        _fixture_cell(partial / "cells" / name, 120 if name == "A0-mid" else 100,
                      120 if name == "A0-mid" else 100)
    (partial / "RESOLUTION-STOP-A0-MID").write_text("A0_MID_DRIFT_STOP\n")
    if analyze(partial)["RUN_VALIDITY_STATE"] != "RESOLUTION_INSUFFICIENT":
        raise EvidenceError("registered mid-anchor stop fixture failed")
    return {"status": "PASS", "matrix": len(CORE), "defect_fixtures": ["complete", "supplement-complete", "supplement-single-missing", "writeback-missing-drain", "missing-direction", "missing-job", "timing-+1s", "timing-+58s", "anchor-drift", "mid-anchor-stop", "early-stop", "pressure-flip"]}
...
def main():
    ...
    else: result = online_cell_summary(args.cell)
