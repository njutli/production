...
RUN_ID=${2:-}
CELL=${3:-}
ROOT=/tmp/production/opencode-04tmp2c-$RUN_ID
...
CACHE_ROOT=$CACHE_PARENT/jfs-04tmp2c-$RUN_ID
CEPH_CONF=/etc/ceph/ceph.conf
METRICS_ADDR=127.0.0.1:9568
EXPECTED_UID=1002
EXPECTED_GID=1002
BLOCK_BYTES=$((256 * 1024))
WORKSET_BYTES=$((16 * 1024 * 1024 * 1024))
SELF_DIR=$(cd -- "$(dirname -- "$0")" && pwd)
SCRUB_CONTROL=$SELF_DIR/u141d-scrub-control.sh

die() { printf 'E_04TMP2C\t%s\n' "$*" >&2; exit 42; }

valid_run() {
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ ]] || die "invalid RUN_ID"
  [[ $ROOT == "/tmp/production/opencode-04tmp2c-$RUN_ID" && $ROOT != / && $ROOT != *..* ]] || die "unsafe result root"
  [[ $CACHE_ROOT == "/mnt/jfs-cache/jfs-04tmp2c-$RUN_ID" && $CACHE_ROOT != / && $CACHE_ROOT != *..* ]] || die "unsafe cache root"
  [[ ! -L $ROOT && ! -L $CACHE_ROOT ]] || die "symlink root refused"
}

cell_fields() {
  [[ $CELL == A0-pre || $CELL == A0-post || $CELL =~ ^C(02|04|08|16|32)$ ]] || die "invalid cell: $CELL"
  case $CELL in
    A0-pre|A0-post) TIER_GIB=0; TIER_MIB=0; CACHE_DIR= ;;
    C02) TIER_GIB=2; TIER_MIB=2048; CACHE_DIR=$CACHE_ROOT/cache-C02 ;;
    C04) TIER_GIB=4; TIER_MIB=4096; CACHE_DIR=$CACHE_ROOT/cache-C04 ;;
    C08) TIER_GIB=8; TIER_MIB=8192; CACHE_DIR=$CACHE_ROOT/cache-C08 ;;
    C16) TIER_GIB=16; TIER_MIB=16384; CACHE_DIR=$CACHE_ROOT/cache-C16 ;;
    C32) TIER_GIB=32; TIER_MIB=32768; CACHE_DIR=$CACHE_ROOT/cache-C32 ;;
  esac
  JFS_MNT=/tmp/jfs-04tmp2c-mnt-$RUN_ID-$CELL
  CELL_ROOT=$ROOT/cells/$CELL
}

validate_cell_paths() {
  cell_fields
  [[ $JFS_MNT == "/tmp/jfs-04tmp2c-mnt-$RUN_ID-$CELL" && $JFS_MNT != / && $JFS_MNT != *..* ]] || die "unsafe mount path"
...
asset_manifest() {
  local out=$1 base=$2
  [[ -d $base/test_dir && ! -L $base/test_dir ]] || die "dataset directory missing: $base"
  find "$base/test_dir" -maxdepth 1 -type f -name 'read_test.*.0' -printf '%f\t%i\t%s\t%T@\n' | sort -V >"$out"
  [[ $(wc -l <"$out") -eq 128 ]] || die "dataset count is not 128"
  awk -F '\t' '$3 != 1073741824 { bad=1 } END { exit bad }' "$out" || die "dataset size is not 1GiB"
}
...
verify_runtime_health() {
  ...
  ceph -s --format json >"$out/ceph-status.json" || die "Ceph status unavailable"
...
mount_process_gate() {
  ...
  python3 - "$JFS" "$CELL_ROOT/mount-pids-pre.txt" "$cache_arg" "$TIER_MIB" "$METRICS_ADDR" <<'PY'
import os, sys
...
cmd = workers[0][3]
required = ['--read-only', '--prefetch 0', '--max-uploads 150', '--max-fuse-io 256K', f'--metrics {metrics}']
if cache:
...
run_fio() {
  ...
  write_jobfile "$out/randread.fio"
  local -a cmd=(fio --readonly "$out/randread.fio" --output-format=json --output="$out/fio.json"
    --write_bw_log="$out/bw/randread" --log_avg_msec=1000 --per_job_logs=1)
...
jobs=d.get('jobs', [])
if len(jobs) != 128:
    raise SystemExit(f'expected 128 fio jobs, got {len(jobs)}')
...
mount_jfs() {
  ...
  local -a cmd=("$JFS" mount -d --read-only --prefetch 0 --max-uploads 150 --max-fuse-io 256K)
  if [[ -n $CACHE_DIR ]]; then
...
cmd_inventory() {
  ...
  [[ -r $CEPH_CONF ]] || die "CEPH_CONF missing"
  ...
  ceph -s --format json >"$ROOT/inventory/ceph-status.json" || die "Ceph status unavailable"
  for cell in A0-pre C02 C04 C08 C16 C32 A0-post; do
    CELL=$cell; cell_fields
    [[ ! -e $JFS_MNT && ! -e $CELL_ROOT ]] || die "cell path already exists: $cell"
  done
  printf 'INVENTORY_PASS\n' >"$ROOT/inventory/PASS"
  printf '04TMP2C_INVENTORY_PASS root=%s\n' "$ROOT"
}
...
cell	cache_mib	cache_fraction	cache_dir
A0-pre	0	0	NONE
C02	2048	0.125	RUN/cache-C02
C04	4096	0.25	RUN/cache-C04
C08	8192	0.50	RUN/cache-C08
C16	16384	1.00	RUN/cache-C16
C32	32768	2.00	RUN/cache-C32
A0-post	0	0	NONE
EOF
...
EOF
  printf 'A0-pre,C02,C04,C08,C16,C32,A0-post\n' >"$ROOT/plans/matrix-order.txt"
  sha256sum "$0" "$(dirname "$0")/t04tmp2c-cache-analyze.py" "$(dirname "$0")/t04tmp2c-cache-gate0-offline.sh" >"$ROOT/plans/scripts.sha256"
  printf 'PLAN_PASS\n' >"$ROOT/plans/PASS"
  printf '04TMP2C_PLAN_PASS root=%s\n' "$ROOT"
}

cmd_run_cell() {
  valid_run; validate_cell_paths
  [[ ${TMP2C_ACK:-} == "I_ACK_04TMP2C_$RUN_ID" ]] || die "exact execution ACK missing"
  ...
  cleanup_cache_cell
  printf 'CELL_PASS\t%s\n' "$CELL" >"$CELL_ROOT/PASS"
  printf '04TMP2C_CELL_PASS cell=%s\n' "$CELL"
}
...
cmd_offline_self_test() {
  ...
  local cells='A0-pre C02 C04 C08 C16 C32 A0-post' cell count=0
  for cell in $cells; do CELL=$cell; cell_fields; validate_cell_paths; count=$((count+1)); done
  [[ $count -eq 7 && $WORKSET_BYTES -eq $((128*128*1024*1024)) ]] || die "matrix/workset contract mismatch"
  printf '04TMP2C_OFFLINE_SELF_TEST_PASS cells=%s workset_bytes=%s\n' "$count" "$WORKSET_BYTES"
}
