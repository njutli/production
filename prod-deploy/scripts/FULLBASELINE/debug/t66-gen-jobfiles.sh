...
OUT=${1:-}
TEST_DIR=${2:-}
[[ "$OUT" == /tmp/production/* && "$TEST_DIR" =~ ^/tmp/jfs-t66-[0-9]{8}-[0-9]{6}-mnt-(R0[1-8]|ARM-CANARY-(B1c|D1)|SEED-FORMAL|RESTORE-(CANARY|PREFLIGHT)-(B1c|D1))/(test_dir|seed_layout)$ ]] || {
  printf 'E_T66\tunsafe output or test directory\n' >&2
  exit 42
}
mkdir -p "$OUT"
