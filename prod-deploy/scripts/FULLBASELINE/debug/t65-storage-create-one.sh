#!/usr/bin/env bash
# Create the three persistent NVMe-backed files for one 03-22b node.
...
PRE_MIN=$((768 * GIB))
POST_MIN=$((512 * GIB))
ROLES=(a1-shared b1-kv b1-logs)
BYTES=$((128 * GIB))
SIZES=($BYTES $((96 * GIB)) $((32 * GIB)))
FILES=("$BACKING_ROOT/a1-shared.img" "$BACKING_ROOT/b1-kv.img" "$BACKING_ROOT/b1-logs.img")

t65_assert_abs_scoped_path "$BACKING_ROOT" "$RUN_ID"
t65_assert_abs_scoped_path "$MOUNT_ROOT" "$RUN_ID"
t65_assert_no_production_overlap "$BACKING_ROOT"
...
mount_identity() {
  ...
  [[ "$source" == /dev/nvme1n1 && "$target" == /mnt/jfs-tikv && "$fstype" == ext4 ]] ||
    t65_die "production TiKV mount mismatch: source=$source target=$target fstype=$fstype"
}
...
assert_clean_scope() {
  ...
  t65_require_absent "$STATE"; t65_require_absent "$BACKING_ROOT"; t65_require_absent "$MOUNT_ROOT"
  ! findmnt -rn -o TARGET | awk -v p="$MOUNT_ROOT" '$1==p || index($1,p"/")==1{found=1} END{exit !found}' ||
    t65_die 't65 mount already exists'
  ! sudo losetup -l -n -O BACK-FILE | awk -v p="$BACKING_ROOT/" 'index($1,p)==1{found=1} END{exit !found}' ||
    t65_die 't65 loop already exists'
}

preflight() {
  local avail mem_kib
  t65_require_tools awk df du fallocate findmnt hostname losetup realpath stat sudo
  host_guard
  mount_identity
  assert_clean_scope
  avail=$(available_bytes)
  [[ "$avail" =~ ^[0-9]+$ ]] || t65_die 'invalid df available value'
  t65_capacity_pre_ok "$avail" || t65_die "Avail_pre below 768GiB: $avail"
  mem_kib=$(awk '/^MemAvailable:/{print $2}' /proc/meminfo)
  (( mem_kib >= 67108864 )) || t65_die "MemAvailable below 64GiB: ${mem_kib}KiB"
...
create() {
  local expected="03-22b-storage-create-${RUN_ID}-${NODE_IP}" avail_pre avail_post i path inode dev
  t65_check_auth "${T65_STORAGE_CREATE_AUTH:-}" "$expected"
  t65_record_authorization "$RUN_ID" storage-create "$expected"
  preflight
  ...
  for i in "${!FILES[@]}"; do
    path=${FILES[$i]}
    t65_assert_realpath_exact "$path" "$path"
    t65_assert_no_production_overlap "$path"
    [[ ! -e "$path" ]] || t65_die "backing already exists: $path"
    : > "$path"
    ...
    t65_assert_allocated_file "$path" "${SIZES[$i]}" >> "$STATE.allocation-checks"
    printf 'allocated\t%s\t%s\t%s\t%s\t%s\n' "${ROLES[$i]}" "$path" "${SIZES[$i]}" "$dev" "$inode" >> "$STATE"
  done
  avail_post=$(available_bytes)
  t65_capacity_post_ok "$avail_post" || t65_die "Avail_post below 512GiB after allocation: $avail_post"
...
verify() {
  ...
  [[ -s "$STATE" ]] || t65_die "missing storage state: $STATE"
  t65_validate_storage_contract_rows "$STATE" "$RUN_ID" "$NODE_IP" || t65_die 'storage state text contract mismatch'
  [[ $(t65_state_value "$STATE" meta) == "$RUN_ID" && $(t65_state_value "$STATE" node) == "$NODE_IP" &&
     $(t65_state_value "$STATE" backing_root) == "$BACKING_ROOT" && $(t65_state_value "$STATE" mount_root) == "$MOUNT_ROOT" ]] ||
    t65_die 'storage state identity mismatch'
  mount_identity
  for i in "${!FILES[@]}"; do
    ...
    read -r dev inode < <(awk -F '\t' -v r="$role" '$1=="allocated"&&$2==r{d=$5;i=$6;n++} END{if(n==1)print d,i;else exit 1}' "$STATE") ||
      t65_die "missing unique allocated row: $role"
    actual_dev=$(stat -Lc '%d' -- "$path"); actual_inode=$(stat -Lc '%i' -- "$path")
    [[ "$actual_dev" == "$dev" && "$actual_inode" == "$inode" ]] || t65_die "backing inode changed: $role"
    t65_assert_allocated_file "$path" "$expected_bytes"
  done
  allocated_count=$(awk -F '\t' '$1=="allocated"{n++} END{print n+0}' "$STATE")
  [[ "$allocated_count" -eq 3 ]] || t65_die "allocated row count mismatch: $allocated_count"
...
case "$ACTION" in
  ...
  *) t65_die 'usage: t65-storage-create-one.sh plan|preflight|create|verify RUN_ID NODE_IP';;
esac
