...
SCRIPT_DIR=$THIS_DIR
MODE=${1:-}; RUN_ID=${2:-}
MATRIX_PROFILE=range-flush-abba
ROOT=/tmp/production/opencode-06-2b-$RUN_ID
PLAN_OUT=${T062B_PLAN_OUT:-/tmp/t06-2b-plan-$RUN_ID}
SCRUB=$SCRIPT_DIR/u141d-scrub-control.sh
ANALYZER=$SCRIPT_DIR/t06-2b-range-flush-analyze.py
GATE0=$SCRIPT_DIR/t06-2b-environment-gate0-offline.sh
readonly CACHE_PARENT=/mnt/jfs-cache/04tmp3
CONTRACT=${T062B_CONTRACT:-}
METRICS_BASE=19630
H_JFS=/tmp/juicefs-1.4.1-patched
C_JFS=$ROOT/bin/juicefs-06-2b-c
T_JFS=$ROOT/bin/juicefs-06-2b-t
...
die() { printf 'T062B_DRIVER_FAIL\t%s\n' "$*" >&2; exit 42; }
valid_run() {
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ && $ROOT == /tmp/production/opencode-06-2b-$RUN_ID && ! -L $ROOT ]] || die invalid_RUN_ID_or_root
}
matrix_rows() {
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    1 H0 H 98304 0 1 2 C1 C 98304 0 1 3 T1 T 98304 0 1 \
    4 T2 T 98304 0 1 5 C2 C 98304 0 1 6 H1 H 98304 0 1
}
select_build() {
  BUILD=$1
  case $BUILD in
    H) JFS=$H_JFS; JFS_MD5=24fae0852051c80ca571cb2f20275d46; JFS_SHA256=8f8b5a911d4f3a91f3f28448599f7465d0624b1338956a5efecfcaeb27d9f3b2; JFS_BUILD_ID=20b7209d124bf5abf2cf24e3bc7aa23a069cb6e9; JFS_VERSION='juicefs version 1.4.1+unknown' ;;
    C) JFS=$C_JFS; JFS_MD5=4ea96bfb923733555221279d0a71083e; JFS_SHA256=b713e595a0fd320217913617098e91107de065285860498079e1777a61b2bbbc; JFS_BUILD_ID=17de3e240ea2e4d0efbf2f25817dc8e33bbb90dc; JFS_VERSION='juicefs version 1.4.1+2026-07-30.0b90c7d' ;;
    T) JFS=$T_JFS; JFS_MD5=2f28b8a4fefa78dc95dfaa8b91f72844; JFS_SHA256=4ffd69637d43a1c33725f540124e7569957cf1c6541cea7fc49b0fa3bf38a4b2; JFS_BUILD_ID=1b9ecbd80e1076136cdac04d462f6443c2e8ef69; JFS_VERSION='juicefs version 1.4.1+2026-07-30.0b90c7d' ;;
    *) die invalid_build ;;
  esac
}
contract_value() {
...
need(d['schema']=='06-2b-v1' and d['status']=='APPROVED','approval')
need(d['run_id']==sys.argv[2] and d['meta']==sys.argv[3],'run_meta')
need(d.get('matrix_profile')=='range-flush-abba','matrix_profile')
...
require_online_ack() {
  valid_run
  local execute_ack=I_ACK_06_2B_PHASE_$RUN_ID build
  [[ ${T062B_EXECUTE_ACK:-} == "$execute_ack" ]] || die execute_ack_missing
  [[ ${T062B_PATH_ACK:-} == I_ACK_06_2B_CACHE_PATHS_$RUN_ID ]] || die path_ack_missing
  [[ ${T062B_SCRUB_ACK:-} == I_ACK_GLOBAL_CEPH_SCRUB_PAUSE ]] || die scrub_ack_missing
  ...
  for kind in DRIVER ANALYZER GATE0 SCRUB CONTRACT; do
    ...
    expected=T062B_${kind}_SHA256; expected=${!expected:-}
  ...
  for build in H C T; do
    select_build "$build"
    [[ -x $JFS && ! -L $JFS && $(md5sum "$JFS" | awk '{print $1}') == "$JFS_MD5" &&
       $(sha256sum "$JFS" | awk '{print $1}') == "$JFS_SHA256" &&
       $(readelf -n "$JFS" 2>/dev/null | awk '/Build ID:/{print $3;exit}') == "$JFS_BUILD_ID" &&
       $($JFS version) == "$JFS_VERSION" ]] || die binary_identity_$build
  done
  select_build H
...
cache_dirs_for_cell() {
  [[ $1 =~ ^(H0|H1|C1|C2|T1|T2|W1|W2|S1)$ ]] || die invalid_cache_cell
  validate_parent "$CACHE_PARENT"
  printf '%s/jfs-06-2b-%s-%s\n' "$CACHE_PARENT" "$RUN_ID" "$1"
}
cache_tree_guard() {
  [[ $1 =~ ^(H0|H1|C1|C2|T1|T2|W1|W2|S1)$ && $2 == "$CACHE_PARENT/jfs-06-2b-$RUN_ID-$1" ]] || die unsafe_cache_child
  validate_parent "$CACHE_PARENT"
...
mount_private() {
  ...
  [[ $cell =~ ^(H0|H1|C1|C2|T1|T2)$ && ( $tag == formal || $tag == verify ) ]] || die mount_tags
  local expected=/tmp/jfs-06-2b-$RUN_ID-$cell
  [[ $tag == formal ]] || expected+=-verify
  [[ $mnt == "$expected" && ! -e $mnt && ! -L $mnt ]] || die private_mount_scope
  no_symlink_components "$mnt" || die mount_symlink
  [[ $(md5sum "$JFS" | awk '{print $1}') == "$JFS_MD5" &&
     $(sha256sum "$JFS" | awk '{print $1}') == "$JFS_SHA256" &&
     $(readelf -n "$JFS" 2>/dev/null | awk '/Build ID:/{print $3;exit}') == "$JFS_BUILD_ID" &&
     $($JFS version) == "$JFS_VERSION" ]] || die binary_drift
  printf 'build\tpath\tmd5\tsha256\tbuild_id\tversion\n%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$BUILD" "$JFS" "$JFS_MD5" "$JFS_SHA256" "$JFS_BUILD_ID" "$JFS_VERSION" >"$c/binary-$tag.tsv"
  mkdir -m 0700 -- "$mnt"
  ...
  # Go rewrites argv after daemonization and may truncate the tail when the
  # executable path is long.  The metrics argument is early and unique; the
  # later log/mount path is not a reliable process marker for C/T builds.
  mount_pid_gate "$c/pids-$tag.before" "$c/mount-process-$tag.tsv" "--metrics $metrics" || die mount_process_identity
  volume_identity "$mnt"
}
eval "$(declare -f graceful_umount | sed '1s/graceful_umount/legacy_graceful_umount/')"
graceful_umount() {
  local cell=$1 tag=$2 mnt=$3 expected=/tmp/jfs-06-2b-$RUN_ID-$1
  [[ $tag == formal ]] || expected+=-verify
...
run_cell() {
  local pos=$1 cell=$2 build=$3 cache_mib=$4 clw=$5 wb=$6 c=$ROOT/cells/$2
  local mnt=/tmp/jfs-06-2b-$RUN_ID-$cell verify=/tmp/jfs-06-2b-$RUN_ID-$cell-verify metrics=127.0.0.1:$((METRICS_BASE+pos)) dirs= avail mem
  select_build "$build"
  CELL_START_AVAIL=
  ...
  dirs=$(cache_dirs_for_cell "$cell"); create_cache_dirs "$cell" "$dirs"
  printf 'cell\t%s\nbuild\t%s\nmount_arm\tB\ncache_mib\t%s\ncache_large_write\t%s\nwriteback\t%s\ncache_dirs\t%s\n' "$cell" "$build" "$cache_mib" "$clw" "$wb" "$dirs" >"$c/state.tsv"
  mount_private "$cell" B formal "$mnt" "$metrics" "$dirs"
  ...
  post_warmup_gate "$cell" "$metrics" "$dirs" B
  ...
  drain_writeback "$cell" "$metrics" "$dirs" B || return $?
...
phase() {
  ...
  [[ -d /tmp/production && -d $ROOT && ! -L $ROOT && -d $ROOT/bin && -d $ROOT/scripts &&
     -d $ROOT/input && -d $ROOT/smoke && ! -e $ROOT/cells && ! -e $ROOT/gate0 &&
     ! -e $ROOT/incidents.tsv && ! -e $ROOT/commands.sh ]] || die result_root_precondition
  [[ $(find "$ROOT" -mindepth 1 -maxdepth 1 -printf '%f\n' | LC_ALL=C sort | paste -sd, -) == bin,input,scripts,smoke ]] \
    || die unexpected_result_root_content
  validate_parent "$CACHE_PARENT"; business_guard || die business_guard
  resource_gate yes || die start_budget
  mkdir -m 0700 -- "$ROOT/cells" "$ROOT/gate0"
  ...
  [[ $(sha256sum "$CONTRACT" | awk '{print $1}') == "$T062B_CONTRACT_SHA256" ]] || die copied_contract_drift
  ...
  SCRUB_LEASE=06-2b-$RUN_ID-phase-a; SCRUB_ACTIVE=1
  ...
  freeze_quiet_baseline
  while IFS=$'\t' read -r pos cell build cache_mib clw wb; do run_cell "$pos" "$cell" "$build" "$cache_mib" "$clw" "$wb"; done < <(matrix_rows)
...
write_plans() {
  ...
  { printf 'position\tcell\tbuild\tcache_mib\tcache_large_write\twriteback\n'; matrix_rows; } >"$out/gate0/mainline-matrix.tsv"
...
offline_self_test() {
  valid_run
  [[ $(matrix_order) == H0,C1,T1,T2,C2,H1 ]] || die test_matrix
  [[ $(matrix_rows | awk -F '\t' '$3=="H"&&$4==98304&&$5==0&&$6==1{n++}END{print n+0}') -eq 2 ]] || die test_H
  [[ $(matrix_rows | awk -F '\t' '$3=="C"&&$4==98304&&$5==0&&$6==1{n++}END{print n+0}') -eq 2 ]] || die test_C
  [[ $(matrix_rows | awk -F '\t' '$3=="T"&&$4==98304&&$5==0&&$6==1{n++}END{print n+0}') -eq 2 ]] || die test_T
  ...
  td=$(mktemp -d "${TMPDIR:-/tmp}/t062b-selftest.XXXXXX")
  (
  ...
  (
    ...
    if (graceful_umount W2 formal /tmp/jfs-06-2b-$RUN_ID-W2) 2>/dev/null; then die test_undrained_umount; fi
    if (cleanup_cache_dirs W2 "$CACHE_PARENT/jfs-06-2b-$RUN_ID-W2") 2>/dev/null; then die test_undrained_cleanup; fi
  ...
  if (cache_dirs_for_cell X1) >/dev/null 2>&1; then die test_invalid_cache_cell; fi
  ...
  printf 'T062B_DRIVER_SELF_TEST_PASS\tprofile=%s\tmatrix=%s\tfixture=%s\n' "$MATRIX_PROFILE" "$(matrix_order)" "$td"
}
case $MODE in
  --self-test) offline_self_test;;
  plan) valid_run; write_plans "$PLAN_OUT"; printf 'T062B_PLAN_ONLY_PASS\troot=%s\n' "$PLAN_OUT";;
  phase) phase;;
