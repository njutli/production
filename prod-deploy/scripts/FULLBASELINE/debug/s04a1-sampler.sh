#!/usr/bin/env bash
# Isolated evidence samplers for 04-2. Modes: node, metrics, client.
set -euo pipefail
export LC_ALL=C
...
case "$OUT" in
  /tmp/production/opencode-04-2-"$RUN_ID"/*|/tmp/jfs-s04a1-"$RUN_ID"-*) ;;
  *) s04a1_die "unsafe sampler OUT: $OUT";;
esac
case "$STOP_FILE" in "$OUT/control.stop") ;; *) s04a1_die 'stop file must be OUT/control.stop';; esac
[[ "$WATCHDOG" =~ ^[0-9]+$ ]] && (( WATCHDOG >= 900 && WATCHDOG <= 1200 )) || s04a1_die 'watchdog must be 900..1200s'
[[ ! -e "$STOP_FILE" ]] || s04a1_die 'sampler stop file already exists'
mkdir -p "$OUT"
...
sample_node() {
  s04a1_node_suffix "$NODE_IP" >/dev/null
  ...
  state="/tmp/s04a1-${RUN_ID}-storage-active-${S04A1_STORAGE_INSTANCE:-$INSTANCE}-${NODE_IP##*.}.tsv"
  work="/tmp/jfs-s04a1-${RUN_ID}-${INSTANCE}-${lower}"
  pidfile="$work/tikv.pid.tsv"
  config="$work/tikv.toml"
  [[ -s "$state" && -s "$pidfile" && -s "$config" ]] || s04a1_die 'node sampler state/config missing'
  IFS=$'\t' read -r pid start md5 cfg < "$pidfile"
  [[ "$cfg" == "$config" && -r /proc/$pid/stat ]] || s04a1_die 'node sampler PID state mismatch'
  actual_start=$(awk '{print $22}' /proc/$pid/stat)
  actual_md5=$(md5sum /proc/$pid/exe | awk '{print $1}')
  [[ "$actual_start" == "$start" && "$actual_md5" == "$md5" ]] || s04a1_die 'node sampler process fingerprint changed'
  mapfile -t DEVS < <(awk -F '\t' '$1=="loop"{print $2}' "$state")
  DEVS+=(/dev/nvme1n1)
  if [[ "$CLUSTER" == C ]]; then expected_devices=1; else expected_devices=2; fi
  (( ${#DEVS[@]} == expected_devices )) || s04a1_die 'unexpected loop device count'
  printf 'node=%s\npid=%s\nstarttime=%s\ndevices=%s\n' "$NODE_IP" "$pid" "$start" "${DEVS[*]}" > "$OUT/node-meta.txt"
  DEADLINE=$((SECONDS + WATCHDOG))
  while keep_sampling; do
    [[ -r /proc/$pid/stat && $(awk '{print $22}' /proc/$pid/stat) == "$start" ]] || s04a1_die 'TiKV process changed during sampling'
    epoch=$(date +%s%N)
    {
      printf 'BEGIN\t%s\t%s\n' "$epoch" "$NODE_IP"
      ...
      while IFS=$'\t' read -r role mnt; do
        # `df -L` is not portable (and is unsupported by the target nodes).
        # Storage state already forbids symlinked roots, so default df path
      ...
      done < <(awk -F '\t' '$1=="data_root"{print "data\t"$2}' "$state")
...
sample_metrics() {
  ...
  while keep_sampling; do
    ...
    for node in "${S04A1_NODES[@]}"; do
      curl -fsS --connect-timeout 3 --max-time 8 "http://${node}:${S04A1_TIKV_STATUS_PORT}/metrics" \
        | gzip > "$dir/${epoch}-${node}.prom.gz"
      printf '%s\t%s\t%s\n' "$epoch" "$node" "$dir/${epoch}-${node}.prom.gz" >> "$OUT/metrics-heartbeat.tsv"
    done
    curl -fsS --connect-timeout 3 --max-time 8 "http://10.20.1.150:${S04A1_PD_CLIENT_PORT}/pd/api/v1/stores" | gzip > "$dir/${epoch}-pd-stores.json.gz"
...
sample_client() {
  local state="/tmp/production/opencode-04-2-${RUN_ID}/instances/${INSTANCE}/volume.tsv"
  local pid start expected_start nic
  [[ -s "$state" ]] || s04a1_die 'client sampler volume state missing'
  pid=$(awk -F '\t' '$1=="pid"{print $2}' "$state")
  start=$(awk -F '\t' '$1=="starttime"{print $2}' "$state")
  nic=${S04A1_CLIENT_NIC:-enp139s0f0np0}
  [[ -r "/sys/class/net/$nic/statistics/rx_bytes" ]] || s04a1_die "NIC missing: $nic"
  DEADLINE=$((SECONDS + WATCHDOG))
  while keep_sampling; do
    [[ -r /proc/$pid/stat ]] || s04a1_die 'JuiceFS process disappeared'
    expected_start=$(awk '{print $22}' /proc/$pid/stat)
    [[ "$expected_start" == "$start" ]] || s04a1_die 'JuiceFS process starttime changed'
    printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$(date +%s%N)" "$nic" \
...
case "$MODE" in
  ...
  *) s04a1_die 'usage: s04a1-sampler.sh node|metrics|client RUN_ID CLUSTER INSTANCE OUT STOP_FILE WATCHDOG [NODE_IP]';;
esac
...
if [[ -s "$STOP_FILE" ]]; then
  ...
  s04a1_die 'sampler watchdog expired before stop signal'
fi
