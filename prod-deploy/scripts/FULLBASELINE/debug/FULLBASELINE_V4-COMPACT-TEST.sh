...
#
# 用法（在 157 上运行）：
#   bash FULLBASELINE_V4-COMPACT-TEST.sh A 180 5 --layout
#   bash FULLBASELINE_V4-COMPACT-TEST.sh B 180 5
#   CACHE_SIZE_GB=100 bash FULLBASELINE_V4-COMPACT-TEST.sh C 180 5
#
# 差异（vs FULLBASELINE_V4.sh）：
#   - aggressive_cleanup 含 juicefs gc --compact（清理切片碎片）
#   - 测试前自动 set_cache_config + reset_state（gc --compact + compact + drop_caches + remount）
#   - CACHE_SIZE_GB 可通过环境变量配置（默认 30）
#
...
gear_stat() {
    local outdir="$1" label="$2" direction=0
    case "${label}" in
        randrw*) direction=0 ;;
        *write*) direction=1 ;;
    esac
    python3 - "${outdir}" "${direction}" > "${outdir}/gear.txt" 2>/dev/null <<'PYEOF' || true
...
for _ in range(50):
    ...
    if done:
        break
g2 = [x for x in core if abs(x - c1) > abs(x - c2)]
print("gear slow=%.0f fast=%.0f sep=%.1f%% fast_frac=%.1f%% stall=%.1f%% p10=%.0f p50=%.0f p90=%.0f cv=%.1f%% n=%d" % (
    c1, c2, (c2 / c1 - 1) * 100 if c1 else 0, 100.0 * len(g2) / len(core), stall,
    sv[int(0.1 * n)], med, sv[int(0.9 * n)],
    statistics.pstdev(v) / statistics.mean(v) * 100, n))
PYEOF
...
for item in items:
    ...
    for r in range(1, repeat + 1):
        ...
        if os.path.exists(os.path.join(subdir, "INVALID.txt")):
            excluded.append("r%d:INVALID" % r)
            continue
        if os.path.exists(os.path.join(subdir, "PG_UNSTABLE.txt")):
            excluded.append("r%d:PG_UNSTABLE" % r)
            continue
    ...
    def report(suffix, vs):
        ...
        print("  %s%s: n=%d median=%.0f max_dev=%.1f%% CV=%.1f%%%s range=%.0f-%.0f MiB/s" % (
            item, suffix, len(vs), med, max_dev, cv, tag, min(vs), max(vs)))
    if vals:
        report("", vals)
        if skip_rounds > 0 and len(vals) > skip_rounds + 1:
            report("[弃前%d轮]" % skip_rounds, vals[skip_rounds:])
        print("    轮值: %s" % " ".join("r%d=%.0f" % (rn, v) for rn, v in zip(rounds, vals)))
    else:
