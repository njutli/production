#!/usr/bin/env bash
# Node-local temporary PD/TiKV lifecycle for 03-22. No sudo is used here.
set -euo pipefail
export LC_ALL=C
...
SUFFIX=$(t64_node_suffix "$NODE_IP")
...
t64_assert_abs_scoped_path "$WORK" "$RUN_ID"

if [[ "$CLUSTER" == A ]]; then
  KV="$BASE/fs-shared/runs/$INSTANCE/kv"
  WAL="$BASE/fs-shared/runs/$INSTANCE/wal"
  RAFT="$BASE/fs-shared/runs/$INSTANCE/raft-engine"
else
  KV="$BASE/fs-kv/runs/$INSTANCE/kv"
  WAL="$BASE/fs-logs/runs/$INSTANCE/wal"
  RAFT="$BASE/fs-logs/runs/$INSTANCE/raft-engine"
fi
PD_DATA="$BASE/pd/runs/$INSTANCE/pd-$SUFFIX"
for path in "$KV" "$WAL" "$RAFT" "$PD_DATA"; do t64_assert_abs_scoped_path "$path" "$RUN_ID"; done
...
render() {
  [[ ${T64_INSTANCE_AUTH:-} == "03-22-instance-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t64_die "set exact T64_INSTANCE_AUTH=03-22-instance-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  [[ -x "$PD_BIN" && -x "$TIKV_BIN" ]] || t64_die 'PD/TiKV binaries missing or not executable'
  [[ -s "/tmp/jfs-t64-${RUN_ID}-${LOWER}-storage.tsv" ]] || t64_die 'storage state missing'
  [[ ! -e "$WORK" ]] || t64_die "instance work directory already exists: $WORK"
  for port in "$T64_PD_CLIENT_PORT" "$T64_PD_PEER_PORT" "$T64_TIKV_PORT" "$T64_TIKV_STATUS_PORT"; do
    port_free "$port" || t64_die "port already busy: $port"
  done
  mkdir -p "$WORK" "$KV" "$WAL" "$RAFT" "$PD_DATA"
  umask 077
  local name="t64-${INSTANCE,,}-${SUFFIX}"
  local initial="t64-${INSTANCE,,}-150=http://10.20.1.150:${T64_PD_PEER_PORT},t64-${INSTANCE,,}-151=http://10.20.1.151:${T64_PD_PEER_PORT},t64-${INSTANCE,,}-152=http://10.20.1.152:${T64_PD_PEER_PORT}"
  printf '%s\n' \
    "name = \"$name\"" \
    "data-dir = \"$PD_DATA\"" \
    "client-urls = \"http://0.0.0.0:${T64_PD_CLIENT_PORT}\"" \
    "advertise-client-urls = \"http://${NODE_IP}:${T64_PD_CLIENT_PORT}\"" \
    "peer-urls = \"http://0.0.0.0:${T64_PD_PEER_PORT}\"" \
    "advertise-peer-urls = \"http://${NODE_IP}:${T64_PD_PEER_PORT}\"" \
    "initial-cluster = \"$initial\"" \
    'initial-cluster-state = "new"' \
    "initial-cluster-token = \"jfs-t64-${RUN_ID}-${INSTANCE}\"" \
    '[log]' 'level = "info"' \
    '[replication]' 'max-replicas = 3' > "$PD_CONFIG"

  printf '%s\n' \
    '[server]' \
    "addr = \"0.0.0.0:${T64_TIKV_PORT}\"" \
    "advertise-addr = \"${NODE_IP}:${T64_TIKV_PORT}\"" \
    "status-addr = \"0.0.0.0:${T64_TIKV_STATUS_PORT}\"" \
    "advertise-status-addr = \"${NODE_IP}:${T64_TIKV_STATUS_PORT}\"" \
    "labels = { host = \"$name\" }" \
    ...
    '[pd]' \
    "endpoints = [\"10.20.1.150:${T64_PD_CLIENT_PORT}\", \"10.20.1.151:${T64_PD_CLIENT_PORT}\", \"10.20.1.152:${T64_PD_CLIENT_PORT}\"]" \
    '[log]' 'level = "info"' \
...
start_one() {
  ...
  case "$service" in
    ...
    *) t64_die 'service must be pd or tikv';;
  esac
  [[ -s "$config" ]] || t64_die "missing config: $config"
  [[ ! -e "$pidfile" ]] || t64_die "pidfile already exists: $pidfile"
  if [[ "$service" == pd ]]; then
    port_free "$T64_PD_CLIENT_PORT" && port_free "$T64_PD_PEER_PORT" || t64_die 'PD port busy'
  else
    port_free "$T64_TIKV_PORT" && port_free "$T64_TIKV_STATUS_PORT" || t64_die 'TiKV port busy'
  fi
...
verify() {
  ...
  pd_pid=$(read_pidfile "$PD_PIDFILE" "$PD_BIN" "$PD_CONFIG") || t64_die 'PD fingerprint invalid'
  tikv_pid=$(read_pidfile "$TIKV_PIDFILE" "$TIKV_BIN" "$TIKV_CONFIG") || t64_die 'TiKV fingerprint invalid'
  curl -fsS --connect-timeout 2 --max-time 5 "http://${NODE_IP}:${T64_PD_CLIENT_PORT}/pd/api/v1/health" >/dev/null || t64_die 'PD health endpoint failed'
  curl -fsS --connect-timeout 2 --max-time 5 "http://${NODE_IP}:${T64_TIKV_STATUS_PORT}/config" > "$WORK/tikv-config-live.json" || t64_die 'TiKV config endpoint failed'
...
stop_one() {
  ...
  pid=$(read_pidfile "$pidfile" "$bin" "$config") || t64_die "$service PID fingerprint invalid; refuse signal"
  kill -TERM "$pid"
  deadline=$((SECONDS + 60))
  while kill -0 "$pid" 2>/dev/null && (( SECONDS < deadline )); do sleep 1; done
  if kill -0 "$pid" 2>/dev/null; then
    t64_die "$service pid=$pid did not exit after TERM; preserved for manual analysis (no KILL sent)"
  fi
...
case "$ACTION" in
  ...
  stop-pd) stop_one pd;;
  *) t64_die 'usage: t64-node-cluster.sh render|start-pd|start-tikv|verify|stop-tikv|stop-pd RUN_ID A|B INSTANCE NODE_IP';;
esac
