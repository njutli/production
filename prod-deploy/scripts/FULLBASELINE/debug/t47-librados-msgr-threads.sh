#!/usr/bin/env bash
# t47-librados-msgr-threads.sh — 03-17c：ms_async_op_threads 标度扫描，只采集、不判定
#
# 2026-08-20 按 03-17b（T48）结论重构，与作废的 03-17 原设计的差别（原任务书 doc/perf-tasks/03-17-*.md 保持原样不动）：
#   1. 删除 librados_thread_count 维度（原 X8 臂）。T48 实测 io_context_pool 线程 8 轮 CPU 恒为 0.00s（F57）。
#   2. 重复单位从"同一挂载内多次 fio"改成"多次独立挂载"。T48 证明方差源是**建连时连接到
#      msgr-worker 的分配抽签**（F56），同一挂载内重复只会复读同一次抽签结果。
#   3. 臂序按轮次轮转（round-robin + 每轮左移），抵消 F55 会话内慢漂移。
#   4. 每轮采逐线程 CPU（msgr-worker-N / io_context_pool），worker CPU 离散度 CV 作为**协变量**落盘。
#      T48 实测 bw~CV r=-0.9907，比 in-flight（r=0.81）强得多。
#   5. ⚑ 取消 ns/B 停机门槛（修 B4-11/B4-12/B4-14）：ns/B 只作为协变量记录，全臂都记，任何值都不停机。
#      判档器 t39-nsbgate.sh 的 verdict 已被 NSB_TOL 抬高关闭（其默认 10% 与文档 18% 不一致，见 B4-14）。
#   6. 只跑 P64（F42b：读饱和点 64 并发流；03-16 锚点 4021.3 也是 P64 同配置），删 P128 与双挂载 CTL。
#   7. ALL DONE 移到压缩包生成之后（修 B4-13）；MANIFEST 改相对路径（修 B4-15）。
#
# 红线：
...
#   - 每个臂挂载后必须核对 msgr-worker 线程数等于请求值，不等即 STOP（本任务的核心闸门）。
...
#
# 三种执行模式：
#   bash t47-librados-msgr-threads.sh --preflight-msgr-only   # 只验证线程注入是否生效，不跑 fio
#   bash t47-librados-msgr-threads.sh --preflight             # 无状态环境清点
#   ACK_SUDO_WRITES=YES bash t47-librados-msgr-threads.sh     # 正式矩阵
#
...
MODE="${1:-full}"
OUT="${OUT:-/tmp/production/opencode-t3.17c}"
...
RUNTIME=180
GAP=20
ANCHOR_MID=4021.3   # 03-16 P64 三轮均值，MiB/s
ANCHOR_TOL_REPORT=10 # 只记录：A3 均值偏离超过它就打 NOTE
ANCHOR_TOL_STOP=25   # 才停机：F56 抽签使单次挂载天然散布 ±15%，窗口必须放宽
PREVERIFY_N=6
...
# 唯一旋钮：ARM_N = ms_async_op_threads。librados_thread_count 维度已按 F57 删除。
# A1 保留作标度锚点：msgr=1 没有分配自由度，是 F56 线性律（1438 MiB/s/满载核）的下端校验点。
# 每个臂在每一轮各挂载一次；ROUNDS 轮 = 每臂 ROUNDS 个独立抽签样本。
declare -a ARMS=(A3 A4 A6 A8 A1)
declare -A ARM_N=([A1]=1 [A3]=3 [A4]=4 [A6]=6 [A8]=8)
ROUNDS="${ROUNDS:-4}"
CFG=P64
IOCTX_DEFAULT=2
...
prepare_arm() { # $1=臂名（A1/A3/A4/A6/A8） $2=轮次
  local arm="$1" pass="$2" n conf want want_io
  ARM="$arm"; PASS="$pass"; ARM_TAG="T47C-$arm-r$pass"
  cleanup_mounts
  ...
  # ⚑ 不再做 ns/B 判档停机（B4-11/B4-12/B4-14）：ns/B 从本轮 P64 的 i1 里算，只当协变量。
  bash "$SNAP" "$OUT" "$ARM_TAG-active" "$META"
  verify_arm || return 1
  log "ARM READY $ARM_TAG threads=$ARM_THREADS_CUR P=$PID_P/$START_P"
}

run_config() { # $1=rep $2=P64（03-17c 只用 P64）
  ...
  PPROF_P=""; PPROF_Q=""
  # pprof 只在第 2 轮的 A3/A8 取，用于对比 3 vs 8 worker 下的排队结构
  if [[ "$PASS" == 2 && ( "$ARM" == A3 || "$ARM" == A8 ) ]]; then
  ...
  # 批次锚点样本：A3 臂的 P64 与 03-16 完全同配置
  if [[ "$ARM" == A3 && "$cfg" == P64 ]]; then
    printf '%s\t%s\n' "$tag" "$bw_p" >> "$OUT/anchor-a3-p64.tsv"
  fi
  # ---------- 协变量（只记录，绝不据此停机）----------
  ...
  [[ "$nsb" =~ ^[0-9]+([.][0-9]+)?$ ]] || nsb=NA
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$tag" "$ARM" "${ARM_N[$ARM]}" "$PASS" "${bw_p:-NA}" "$nsb" "$cvline" >> "$OUT/arm-covariates.tsv"
  log "COVAR $tag msgr=${ARM_N[$ARM]} bw=${bw_p:-NA} ns/B=$nsb worker[cv%/max%core/sum_core/n]=$cvline"
...
anchor_check() { # $1=期望样本数 $2=停机容差百分比 $3=阶段名
  ...
  n=$(wc -l < "$OUT/anchor-a3-p64.tsv" 2>/dev/null || echo 0)
  [[ "$n" -ge "$want" ]] || { log "STOP 锚点样本不足：$n < $want（$stage）"; return 1; }
  mean=$(awk -F'\t' '{s+=$2;c++} END{if(c)printf "%.1f",s/c}' "$OUT/anchor-a3-p64.tsv")
  ...
  awk -v v="$mean" -v a="$rlo" -v b="$rhi" 'BEGIN{exit !(v>=a && v<=b)}' \
    || log "NOTE 批次锚点偏离 ±${ANCHOR_TOL_REPORT}% 窗口（$stage）：A3/P64 mean=$mean 不在 $rlo~$rhi；F56 抽签方差可致，只记录不停机"
  awk -v v="$mean" -v a="$lo" -v b="$hi" 'BEGIN{exit !(v>=a && v<=b)}' || {
    log "STOP 批次锚点严重失配（$stage）：A3/P64 mean=$mean 不在 $lo~$hi（03-16 基准 $ANCHOR_MID ±${tol}%）；臂间比较不可用"
    return 1
  }
  log "锚点通过（$stage）：A3/P64 n=$n mean=$mean，停机窗口 $lo~$hi"
}

run_arm() { # $1=臂 $2=轮次；每次调用 = 一次独立挂载 + 一次 P64（F56：重复单位必须是挂载）
  prepare_arm "$1" "$2" || return 1
  run_config 1 "$CFG" || return 1
...
# 分析方主表：每行 = 一次独立挂载的一次 P64。ns/B 与 worker CPU 全是协变量，无判定列。
printf 'tag\tarm\tmsgr_threads\tround\tbw_mib\tns_per_B\tworker_cv_pct\tmax_worker_pct_core\tmsgr_sum_core\tn_worker\n' \
...
log "矩阵开始：臂 ${ARMS[*]}（ms_async_op_threads=3/4/6/8 + A1 标度锚点），共 $ROUNDS 轮"
log "每轮每臂各一次独立挂载 + 一次 $CFG（F56：方差源是建连抽签，重复单位必须是挂载而不是 fio）"
...
for r in $(seq 1 "$ROUNDS"); do
  ...
  [[ "$r" != 1 ]] || anchor_check 1 "$ANCHOR_TOL_STOP" "round1-A3"
done
anchor_check "$ROUNDS" "$ANCHOR_TOL_STOP" "final-A3"
