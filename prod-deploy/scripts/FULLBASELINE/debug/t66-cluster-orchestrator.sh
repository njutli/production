...
t66_check_run_id "$RUN_ID"; t66_check_cluster "$CLUSTER"; t66_check_instance "$INSTANCE"
[[ $(t66_expected_cluster "$INSTANCE") == "$CLUSTER" ]] || t66_die 'instance/cluster mapping mismatch'
case "$ACTION" in render|start-pd|start-tikv|verify|stop-tikv|stop-pd) ;; *) t66_die 'invalid cluster action';; esac
t66_make_ssh_array; t66_require_tools sshpass ssh curl python3 sha256sum

OUT="/tmp/production/opencode-t3.22c-${RUN_ID}/orchestration"
...
ssh_node() { local node=$1; shift; "${T66_SSH[@]}" "$node" "$@"; }
verify_remote_helpers() {
  local node=$1
  local name local_sha remote_sha
  for name in t66-common.sh t66-node-cluster.sh; do
    local_sha=$(sha256sum "$SCRIPT_DIR/$name" | awk '{print $1}')
    remote_sha=$(ssh_node "$node" "sha256sum '$REMOTE_DIR/$name'" | awk '{print $1}')
    [[ "$local_sha" == "$remote_sha" ]] || t66_die "remote helper SHA mismatch: node=$node file=$name"
  done
}
...
[[ -s "/tmp/jfs-t66-${run}-${arm}-${instance}-activation.tsv" ]] || { echo activation_state_missing >&2; exit 42; }
...
cluster_action() {
  local sub=$1 node auth="03-22c-instance-${RUN_ID}-${INSTANCE}-${CLUSTER}" action_auth
  for node in "${T66_NODES[@]}"; do
    ...
    if [[ "$sub" == render ]]; then
      ssh_node "$node" "export T66_INSTANCE_AUTH='$auth'; bash '$REMOTE_DIR/t66-node-cluster.sh' render '$RUN_ID' '$CLUSTER' '$INSTANCE' '$node'" | tee -a "$LOG"
    elif [[ "$sub" == start-pd || "$sub" == start-tikv || "$sub" == stop-tikv || "$sub" == stop-pd ]]; then
      action_auth="03-22c-cluster-${sub%-*}-${sub#*-}-${RUN_ID}-${CLUSTER}-${INSTANCE}"
      t66_check_auth "${T66_CLUSTER_ACTION_AUTH:-}" "$action_auth"
      ssh_node "$node" "export T66_CLUSTER_ACTION_AUTH='$action_auth'; bash '$REMOTE_DIR/t66-node-cluster.sh' '$sub' '$RUN_ID' '$CLUSTER' '$INSTANCE' '$node'" | tee -a "$LOG"
    else
      ssh_node "$node" "bash '$REMOTE_DIR/t66-node-cluster.sh' '$sub' '$RUN_ID' '$CLUSTER' '$INSTANCE' '$node'" | tee -a "$LOG"
    fi
  done
}
...
verify_global() {
  ...
  while true; do
    ...
    if curl -fsS --connect-timeout 3 --max-time 10 "http://10.20.1.150:${T66_PD_CLIENT_PORT}/pd/api/v1/health" > "$health" &&
      python3 - "$health" <<'PY'
import json,sys
d=json.load(open(sys.argv[1])); assert isinstance(d,list) and len(d)==3 and all(x.get('health') is True for x in d)
PY
    then health_ok=1; fi
    if curl -fsS --connect-timeout 3 --max-time 10 "http://10.20.1.150:${T66_PD_CLIENT_PORT}/pd/api/v1/stores" > "$stores" &&
      python3 - "$stores" <<'PY'
import json,sys
...
PY
    ...
    ((stable>=3)) && break
    ((SECONDS<deadline)) || t66_die "global readiness timeout; evidence=$ready"
    sleep 10
  done
