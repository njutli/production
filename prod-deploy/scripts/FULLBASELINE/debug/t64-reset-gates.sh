#!/usr/bin/env bash
# Ceph/TiKV stability gates for one 03-22 arm. No cluster or volume lifecycle here.
set -euo pipefail
export LC_ALL=C
...
[[ "$INSTANCE" != SMOKE-* ]] || t64_die 'smoke instances do not run reset gates'
if [[ "$ACTION" == post-abort-final-destroy ]]; then
  [[ ${T64_ABORT_RESET_AUTH:-} == "03-22-abort-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t64_die "set exact T64_ABORT_RESET_AUTH=03-22-abort-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  [[ -z ${T64_RESET_AUTH:-} ]] || t64_die 'normal reset authorization is forbidden for abort cleanup'
else
  [[ ${T64_RESET_AUTH:-} == "03-22-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t64_die "set exact T64_RESET_AUTH=03-22-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}"
fi
...
SEED_DIR=''
case "$INSTANCE" in
  SEED-CANARY|SEED-FORMAL|RESTORE-CANARY|RESTORE-PREFLIGHT|GC-CANARY|GC-PREFLIGHT|G0[1-8]|R0[1-8])
    FLAVOR=$(t64_seed_flavor "$INSTANCE")
    SEED_DIR=$(t64_seed_dir "$RUN_ID" "$FLAVOR")
    ;;
esac
[[ ! -e "$RESET" ]] || t64_die "reset output already exists; preserve first attempt: $RESET"
mkdir -p "$RESET"
t64_require_tools "$T64_JUICEFS_BIN" python3 curl sudo sshpass ssh
t64_make_ssh_array
export CEPH_CONF="$PRIVATE_CONF"

[[ $(sudo ceph health) == HEALTH_OK ]] || t64_die 'Ceph health is not HEALTH_OK'
! pgrep -x fio >/dev/null || t64_die 'foreign fio exists on client'
for node in "${T64_NODES[@]}"; do
  ! "${T64_SSH[@]}" "$node" 'pgrep -x fio' >/dev/null 2>&1 || t64_die "foreign fio exists on $node"
  mem_kib=$("${T64_SSH[@]}" "$node" "awk '/^MemAvailable:/{print \$2}' /proc/meminfo")
  [[ "$mem_kib" =~ ^[0-9]+$ ]] && (( mem_kib >= 402653184 )) ||
    t64_die "MemAvailable below 384GiB on $node: ${mem_kib:-invalid}KiB"
done
...
pending_total() {
  curl -fsS --connect-timeout 3 --max-time 8 "http://$1:${T64_TIKV_STATUS_PORT}/metrics" |
...
(( ${#OSD_IDS[@]} == 6 )) || t64_die "expected frozen six OSDs, got ${#OSD_IDS[@]}"
printf '%s\n' "${OSD_IDS[@]}" > "$RESET/osd-ids.txt"
sudo ceph osd dump > "$RESET/osd-dump.txt"

case "$ACTION" in
  prepare)
    [[ -s "$OUT/LAYOUT_PASS" && -s "$OUT/volume.tsv" ]] || t64_die 'layout or mounted volume state missing'
    pool_objects > "$RESET/pool-before.tsv"
    compact_and_wait pre-gc || t64_die 'pre-GC OSD cooldown failed'
    printf '%q gc --compact %q\n' "$T64_JUICEFS_BIN" "$META" >> "$OUT/commands.sh"
    "$T64_JUICEFS_BIN" gc --compact "$META" > "$RESET/juicefs-gc.log" 2>&1 || t64_die 'temporary-volume gc --compact failed'
    compact_and_wait post-gc || t64_die 'post-GC OSD cooldown failed'
    tikv_idle_gate || t64_die 'temporary TiKV pending compaction did not reach three consecutive zeros'
    sleep 60
    [[ $(sudo ceph health) == HEALTH_OK ]] || t64_die 'Ceph changed after quiet period'
    ...
    ;;
  post-destroy)
    [[ -s "$OUT/volume.destroyed.tsv" && ! -e "$OUT/volume.tsv" ]] || t64_die 'volume destroy evidence missing'
    compact_and_wait post-destroy || t64_die 'post-destroy OSD cooldown failed'
...
PY
    ...
    ;;
  seed-return)
    [[ "$INSTANCE" == GC-CANARY || "$INSTANCE" == GC-PREFLIGHT || "$INSTANCE" =~ ^G0[1-8]$ ]] ||
      t64_die 'seed-return requires a dedicated GC instance'
    [[ -s "$SEED_DIR/pool-seed.tsv" && -s "$SEED_DIR/gc-baseline.tsv" ]] || t64_die 'seed baselines missing'
    if [[ "$INSTANCE" == GC-PREFLIGHT ]]; then
      [[ -s "$OUT/GC_INSPECT_PASS" && $(awk -F '\t' '$1=="leaked"{print $2}' "$OUT/gc-inspect.tsv") == 0 ]] ||
        t64_die 'GC-PREFLIGHT requires a zero-leak inspect result'
    else
      [[ -s "$OUT/GC_DELETE_PASS" && -s "$OUT/gc-postcheck.tsv" ]] || t64_die 'GC delete/postcheck evidence missing'
    fi
    compact_and_wait seed-return || t64_die 'post-GC OSD cooldown failed'
...
PY
    ...
    ;;
  post-final-destroy)
    [[ "$INSTANCE" == GC-CANARY || "$INSTANCE" == G08 ]] || t64_die 'post-final-destroy requires GC-CANARY or G08'
    [[ -s "$SEED_DIR/seed.destroyed.tsv" && -s "$SEED_DIR/pool-pre-format.tsv" ]] || t64_die 'seed destroy/pre-format evidence missing'
    compact_and_wait final-destroy || t64_die 'post-destroy OSD cooldown failed'
...
PY
  ...
  post-abort-final-destroy)
    [[ "$INSTANCE" =~ ^G0[1-7]$ ]] || t64_die 'post-abort-final-destroy requires an early G01..G07 cleanup instance'
    [[ -s "$OUT/ABORT_SEED_DESTROY_PASS" && -s "$SEED_DIR/seed.destroyed.tsv" &&
      ...
      t64_die 'abort seed destroy evidence or pre-format baseline missing'
    compact_and_wait abort-final-destroy || t64_die 'post-abort-destroy OSD cooldown failed'
...
PY
    ...
    ;;
  *) t64_die 'usage: t64-reset-gates.sh prepare|post-destroy|seed-return|post-final-destroy|post-abort-final-destroy RUN_ID A|B INSTANCE';;
esac
