...
t66_make_ssh_array
t66_require_tools sshpass ssh sha256sum awk python3
OUT="/tmp/production/opencode-t3.22c-${RUN_ID}/inventory"
[[ ! -e "$OUT/INVENTORY_PASS" ]] || t66_die 'inventory already completed for this RUN_ID'
mkdir -p "$OUT/nodes"

remote_inventory() {
  local node=$1
  "${T66_SSH[@]}" "$node" bash -s -- "$node" <<'REMOTE'
set -euo pipefail
export LC_ALL=C
expected=$1
...
awk '/^(pswpin|pswpout) /{print "vmstat\t"$1"\t"$2}' /proc/vmstat
printf 'section\tnuma-memory\n'
for f in /sys/devices/system/node/node[0-9]*/meminfo; do [[ -r "$f" ]] && sed "s#^#${f}\t#" "$f"; done
...
printf 'section\tnvme-smart\n'
command -v smartctl
smartctl --version | head -n 2
sudo smartctl -a -j /dev/nvme1n1
printf 'section\tports\n'
for port in 2379 2380 20160 20180 12379 12380 30160 30180; do ss -Hlnpt "sport = :$port" || true; done
printf 'section\tt66-residue\n'
findmnt -rn -o SOURCE,TARGET,FSTYPE,OPTIONS | awk '$2 ~ /^\/mnt\/jfs-t6[456]-/{print}'
sudo losetup -l -n -O NAME,BACK-FILE | awk '$2 ~ /\/jfs-t6[456]-/{print}'
# All t64/t65/t66 lifecycle roots are direct children of one of these three
...
for root in /mnt /mnt/jfs-tikv /tmp; do
  find "$root" -maxdepth 1 -mindepth 1 \( -name 'jfs-t64-*' -o -name 'jfs-t65-*' -o -name 'jfs-t66-*' \) -printf '%y\t%p\t%u:%g\t%m\n'
done | sort
ps -eo pid=,ppid=,comm=,args= | awk '$3=="fio" || $0 ~ /jfs-t6[456]-/{print}'
...
[[ $(<"$OUT/ceph-health.txt") == HEALTH_OK ]] || t66_die 'Ceph is not HEALTH_OK'
...
for p in nodes:
    ...
    mem=re.search(r'^mem\tMemAvailable:\t(\d+)$',text,re.M)
    assert mem
    mem_avails.append(int(mem.group(1)))
...
assert min(mem_avails)>=128*1024**2, mem_avails
(root/'contract.tsv').write_text(
    f'uid\t{uids[0][0]}\n' f'gid\t{uids[0][1]}\n'
    f'min_avail_pre\t{min(avails)}\n' f'min_mem_available_kib\t{min(mem_avails)}\n'
    f'planned_nvme_bytes_per_node\t{128*1024**3}\n'
    f'd1_ram_backing_bytes_per_node\t{32*1024**3}\n'
    f'production_reserve\t{640*1024**3}\n')
PY
