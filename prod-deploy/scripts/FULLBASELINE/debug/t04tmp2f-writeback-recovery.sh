...
die(){ printf 'E_04TMP2F_RECOVERY\t%s\n' "$*" >&2; exit 42; }
valid(){
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ ]] || die invalid_RUN_ID
  [[ $LABEL =~ ^after-W(20|32|64|96|128)-randwrite$ ]] || die invalid_label
  [[ $ROOT == /tmp/production/opencode-04tmp2f-$RUN_ID && $ROOT != / && $ROOT != *..* ]] || die unsafe_root
...
cmd_run(){
  valid
  [[ ${TMP2F_RECOVERY_ACK:-} == I_ACK_04TMP2F_RECOVERY_$RUN_ID ]] || die ack_missing
  ...
  printf '04TMP2F_RECOVERY_PASS label=%s objects=%s seed=%s\n' "$LABEL" "$after" "$seed"
}
cmd_offline(){
  RUN_ID=20260903-235959; LABEL=after-W20-randwrite
  ROOT=/tmp/production/opencode-04tmp2f-$RUN_ID; CEPH_CONF=$ROOT/inventory/ceph-msgr8.conf; OUT=$ROOT/recovery/$LABEL
  [[ $ROOT != / && $OUT == "$ROOT/recovery/after-W20-randwrite" ]] || die offline_scope
  printf '04TMP2F_RECOVERY_OFFLINE_SELF_TEST_PASS\n'
}
