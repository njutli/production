...
MODE=${1:-}; RUN_ID=${2:-}
case $MODE in
  buffered-self-test|buffered-plan|buffered-phase) MATRIX_PROFILE=buffered-abba ;;
  *) MATRIX_PROFILE=buffered-abba ;;
esac
ROOT=/tmp/production/opencode-06-4-$RUN_ID
PLAN_OUT=${T064_PLAN_OUT:-/tmp/t06-4-plan-$RUN_ID}
SCRUB=$SCRIPT_DIR/u141d-scrub-control.sh
ANALYZER=$SCRIPT_DIR/t06-4-buffered-io-analyze.py
GATE0=$SCRIPT_DIR/t06-4-buffered-io-gate0-offline.sh
CONTRACT=${T064_CONTRACT:-}
METRICS_BASE=19630
...
die() { printf 'T064_DRIVER_FAIL\t%s\n' "$*" >&2; exit 42; }
valid_run() {
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ && $ROOT == /tmp/production/opencode-06-4-$RUN_ID && ! -L $ROOT ]] || die invalid_RUN_ID_or_root
}
matrix_rows() {
  printf '%s\t%s\t%s\t%s\t%s\t%s\n' \
    1 D1 D 0 0 0 2 B1 B 0 0 0 \
    3 B2 B 0 0 0 4 D2 D 0 0 0
}

# 06-4 has no JuiceFS disk cache and no writeback.  D/B differ only in fio's
# buffered/direct model; the cache-parent capacity checks remain inherited.
mount_args() {
  local arm=$1 mnt=$2 metrics=$3 log=$4 dirs=$5
  [[ -z $dirs ]] || die buffered_model_must_not_create_juicefs_cache
  MOUNT_CMD=("$JFS" mount -d --max-fuse-io 256K --buffer-size 300
    --max-uploads 150 --max-downloads 200 --metrics "$metrics" --log "$log"
    --cache-size 0 "$META" "$mnt")
  [[ $arm == D || $arm == B ]] || die invalid_buffered_arm
}

# Warmup is always buffered; formal D cells use direct=1 and B cells direct=0.
write_fio_job() {
  local out=$1 mnt=$2 mode=$3 duration=$4 logs=$5 cell direct
  cell=$(basename "$(dirname "$out")")
  direct=1
  [[ $mode == randread || $cell == B1 || $cell == B2 ]] && direct=0
  mkdir -m 0700 -p "$out"
  [[ $logs == yes ]] && mkdir -m 0700 -p "$out/bw" "$out/clat"
  {
    printf '%s\n' '[global]' 'ioengine=libaio' 'iodepth=128' 'numjobs=128' "rw=$mode" 'rwmixread=50' 'bs=256K' 'filesize=1G' 'size=1G' "direct=$direct" 'invalidate=0' 'fallocate=none' 'allow_file_create=0' 'openfiles=128' 'time_based=1' "runtime=$duration" 'group_reporting=1' 'randrepeat=1' 'randseed=20260915'
...
need(d['schema']=='06-4-v1' and d['status']=='APPROVED','approval')
need(d['run_id']==sys.argv[2] and d['meta']==sys.argv[3],'run_meta')
need(d.get('matrix_profile')=='buffered-abba','matrix_profile')
for k in ('hostname','machine_id','ceph_fsid','volume_uuid','fio_version'):
...
## 06-4 has no JuiceFS disk-cache contract.
s=d['space']
for k in ('minimum_mem_available_bytes','max_fio_wall_seconds','stop_latency_seconds'):
 need(type(s[k]) is int and s[k]>0,'space_'+k)
need(180<=s['max_fio_wall_seconds']<=3600,'wall_guard')
need(1<=s['stop_latency_seconds']<=30,'stop_latency')
...
require_online_ack() {
  valid_run
  local execute_ack=I_ACK_06_4_PHASE_$RUN_ID
  [[ $MATRIX_PROFILE != burst-abba ]] || execute_ack=I_ACK_06_4_BUFFERED_PHASE_$RUN_ID
  [[ ${T064_EXECUTE_ACK:-} == "$execute_ack" ]] || die execute_ack_missing
  [[ ${T064_PATH_ACK:-} == I_ACK_06_4_PRIVATE_MOUNTS_$RUN_ID ]] || die path_ack_missing
  [[ ${T064_SCRUB_ACK:-} == I_ACK_GLOBAL_CEPH_SCRUB_PAUSE ]] || die scrub_ack_missing
  ...
  for kind in DRIVER ANALYZER GATE0 SCRUB CONTRACT; do
    ...
    expected=T064_${kind}_SHA256; expected=${!expected:-}
...
cache_dirs_for_cell() {
  ...
  :
  printf '%s/jfs-06-4-%s-%s\n' "$CACHE_PARENT" "$RUN_ID" "$1"
}
cache_tree_guard() {
  [[ $1 =~ ^(S|W|B)[12]$ && $2 == "$CACHE_PARENT/jfs-06-4-$RUN_ID-$1" ]] || die unsafe_cache_child
...
resource_gate() {
  local mem swap_total swap_free
  read -r mem swap_total swap_free < <(awk '
    /^MemAvailable:/{m=$2*1024}
    /^SwapTotal:/{t=$2*1024}
    /^SwapFree:/{f=$2*1024}
    END{printf "%.0f %.0f %.0f\n",m,t,f}' /proc/meminfo)
  awk -v m="$mem" -v n="$MIN_MEM_AVAIL" -v t="$swap_total" -v f="$swap_free" \
      'BEGIN{exit !(m>=n && (t==0 || f==t))}'
}
backend_snapshot() {
  local out=$1 node
  mkdir -m 0700 -p "$out"
...
mount_private() {
  ...
  [[ $cell =~ ^[ABCDSW][12]$ && ( $tag == formal || $tag == verify ) ]] || die mount_tags
  local expected=/tmp/jfs-06-4-$RUN_ID-$cell
...
graceful_umount() {
  local cell=$1 tag=$2 mnt=$3 expected=/tmp/jfs-06-4-$RUN_ID-$1
  [[ $tag == formal ]] || expected+=-verify
...
fio_checked() {
  local out=$1 cell=$2 seconds=$3 start now rc failed=0 mem cached dirty writeback
  ...
  while kill -0 "$ACTIVE_FIO_PID" 2>/dev/null; do
    now=$(date +%s)
    read -r mem cached dirty writeback < <(awk '
      /^MemAvailable:/{m=$2*1024}/^Cached:/{c=$2}/^Dirty:/{d=$2}/^Writeback:/{w=$2}
      END{printf "%.0f %s %s %s\n",m,c,d,w}' /proc/meminfo)
    printf '%s\t%s\t%s\t%s\t%s\n' "$(date +%s%N)" "$mem" "$cached" "$dirty" "$writeback" >>"$ROOT/cells/$cell/safety.tsv"
    if (( now-start >= seconds )) || ! resource_gate || ! business_guard; then failed=1; event STOP_LOAD "$cell:memory_business_or_timeout"; stop_active_fio; break; fi
...
targeted_fsync() {
  local cell=$1 mnt=$2 c=$ROOT/cells/$1
  python3 - "$mnt" "$c/fsync-files.tsv" "$c/fsync-summary.json" <<'PY'
import json,os,pathlib,sys,time
mnt=pathlib.Path(sys.argv[1]); rows=[]; start=time.monotonic_ns()
for i in range(128):
 p=mnt/'test_dir'/f'rw_test.{i}.0'
 if p.is_symlink() or not p.is_file() or p.stat().st_size != 1073741824:
  raise SystemExit(f'unsafe_fsync_target:{p}')
 t=time.monotonic_ns(); fd=os.open(p,os.O_RDONLY)
 try: os.fsync(fd)
 finally: os.close(fd)
 rows.append((str(p),time.monotonic_ns()-t))
with open(sys.argv[2],'w') as f:
 f.write('path\telapsed_ns\trc\n')
 for p,ns in rows:f.write(f'{p}\t{ns}\t0\n')
with open(sys.argv[3],'w') as f:
 json.dump({'status':'PASS','files':len(rows),'elapsed_ns':time.monotonic_ns()-start},f);f.write('\n')
PY
  : >"$c/FSYNC_PASS"
}
run_cell() {
  local pos=$1 cell=$2 arm=$3 cache_mib=$4 clw=$5 wb=$6 c=$ROOT/cells/$2
  local mnt=/tmp/jfs-06-4-$RUN_ID-$cell verify=/tmp/jfs-06-4-$RUN_ID-$cell-verify metrics=127.0.0.1:$((METRICS_BASE+pos)) dirs= mem cached dirty writeback
  ...
  read -r mem cached dirty writeback < <(awk '
    /^MemAvailable:/{m=$2*1024}/^Cached:/{c=$2}/^Dirty:/{d=$2}/^Writeback:/{w=$2}
    END{printf "%.0f %s %s %s\n",m,c,d,w}' /proc/meminfo)
  printf 'epoch_ns\tMemAvailable_bytes\tCached_kB\tDirty_kB\tWriteback_kB\n%s\t%s\t%s\t%s\t%s\n' "$(date +%s%N)" "$mem" "$cached" "$dirty" "$writeback" >"$c/start-resources.tsv"
  printf 'epoch_ns\tMemAvailable_bytes\tCached_kB\tDirty_kB\tWriteback_kB\n' >"$c/safety.tsv"
  ...
  # Both arms have JuiceFS cache/writeback disabled; B denotes fio buffered I/O,
  # not the inherited 06-3 writeback arm.
  post_warmup_gate "$cell" "$metrics" "$dirs" D
  health_gate "$c/health-pre-formal"
  run_formal "$cell" "$mnt"
  (( FORMAL_RC == 0 )) || { event FIO_FAIL_PRESERVE_MOUNT "$cell:rc=$FORMAL_RC"; return "$FORMAL_RC"; }
  targeted_fsync "$cell" "$mnt" || { event FSYNC_FAIL_PRESERVE_MOUNT "$cell"; return 43; }
  drain_writeback "$cell" "$metrics" "$dirs" D || return $?
  stop_samplers
  graceful_umount "$cell" formal "$mnt"
  mount_private "$cell" D verify "$verify" "$metrics" ""
...
phase() {
  ...
  business_guard || die business_guard
  ...
  [[ $(sha256sum "$CONTRACT" | awk '{print $1}') == "$T064_CONTRACT_SHA256" ]] || die copied_contract_drift
  ...
  SCRUB_LEASE=06-4-$RUN_ID-phase-a; SCRUB_ACTIVE=1
  ...
  python3 "$ANALYZER" analyze "$ROOT" >"$ROOT/analysis.stdout"
  : >"$ROOT/ANALYSIS_PASS"
...
write_plans() {
  ...
  printf '%s\n' 'quiet120 -> per-cell fresh no-cache mount -> read-warmup60 -> recovery30/timeout900 -> formal180 -> exact-128-file-fsync -> upload-drain3zero -> graceful-unmount -> fresh cache0-readback -> graceful-unmount' >"$out/gate0/lifecycle.txt"
  ...
  printf '%s\n' 'FUTURE AUTHORIZED WRITES: exact RUN result/mount mkdir; scrub helper sudo ceph osd set/unset noscrub and nodeep-scrub; four private JFS mount/umount lifecycles; fio overwrites only the frozen 128 test files; targeted fsync of those exact files; exact owned fio PID SIGTERM/SIGKILL safety stop; exact sampler PID SIGTERM.' >"$out/gate0/write-operations.txt"
  write_fio_job "$out/gate0/warmup" '<PRIVATE_MOUNT>' randread 60 no
  write_fio_job "$out/gate0/D1/formal" '<PRIVATE_MOUNT>' randrw 180 yes
  write_fio_job "$out/gate0/B1/formal" '<PRIVATE_MOUNT>' randrw 180 yes
  python3 - "$RUN_ID" "$META" "$MATRIX_PROFILE" >"$out/gate0/contract-template.NOT-APPROVED.json" <<'PY'
import json,sys
d=dict(schema='06-4-v1',status='NOT_APPROVED',run_id=sys.argv[1],meta=sys.argv[2],matrix_profile='buffered-abba',hostname='INVENTORY_REQUIRED',machine_id='INVENTORY_REQUIRED',ceph_fsid='INVENTORY_REQUIRED',volume_uuid='INVENTORY_REQUIRED',fio_version='INVENTORY_REQUIRED',assets_sha256='INVENTORY_REQUIRED',no_concurrent_benchmark=False,approved_osd_flags=[],protected_processes=[],metrics=dict(pending='UNREGISTERED',uploading='juicefs_object_request_uploading',staging_errors='INVENTORY_REQUIRED',error_counter_absent_is_zero=False))
d['space']={'minimum_mem_available_bytes':256*2**30,'max_fio_wall_seconds':300,'stop_latency_seconds':10}
...
offline_self_test() {
  ...
  td=$(mktemp -d "${TMPDIR:-/tmp}/t064-selftest.XXXXXX")
  (
  ...
  (
    ...
    if (graceful_umount W2 formal /tmp/jfs-06-4-$RUN_ID-W2) 2>/dev/null; then die test_undrained_umount; fi
    if (cleanup_cache_dirs W2 "$CACHE_PARENT/jfs-06-4-$RUN_ID-W2") 2>/dev/null; then die test_undrained_cleanup; fi
  ...
  printf 'T064_DRIVER_SELF_TEST_PASS\tprofile=%s\tmatrix=%s\tfixture=%s\n' "$MATRIX_PROFILE" "$(matrix_order)" "$td"
}
# The inherited 06-3 mount_args definition is intentionally replaced here:
# both 06-4 arms use one private no-cache mount.
mount_args() {
  local arm=$1 mnt=$2 metrics=$3 log=$4 dirs=$5
  [[ $arm == D || $arm == B ]] || die invalid_buffered_arm
  [[ -z $dirs ]] || die buffered_model_must_not_create_juicefs_cache
...
offline_self_test_064() {
  valid_run
  [[ $(matrix_order) == D1,B1,B2,D2 ]] || die buffered_matrix
  [[ $(matrix_rows | awk -F '\t' '$3=="D"{n++}END{print n+0}') == 2 ]] || die direct_cells
  [[ $(matrix_rows | awk -F '\t' '$3=="B"{n++}END{print n+0}') == 2 ]] || die buffered_cells
  local d args
  for d in D B; do
    mount_args "$d" /mock/mount 127.0.0.1:19631 /mock/log ''
    args=" ${MOUNT_CMD[*]} "
    [[ $args == *' --cache-size 0 '* ]] || die cache_not_disabled
    [[ $args != *' --writeback '* && $args != *' --cache-large-write '* ]] || die writeback_enabled
  done
  local td
  td=$(mktemp -d "${TMPDIR:-/tmp}/t064-driver-selftest.XXXXXX")
  mkdir -m 0700 -p "$td/D1/formal" "$td/B1/formal"
  write_fio_job "$td/D1/formal" '<PRIVATE_MOUNT>' randrw 180 yes
  write_fio_job "$td/B1/formal" '<PRIVATE_MOUNT>' randrw 180 yes
  grep -Fxq 'direct=1' "$td/D1/formal/fio.job" || die direct_job
  grep -Fxq 'direct=0' "$td/B1/formal/fio.job" || die buffered_job
  grep -Fxq 'invalidate=0' "$td/D1/formal/fio.job" || die invalidate_contract
  if grep -qE '(^|[[:space:]])(ssh|scp|sudo|ceph|fio|juicefs|mount|umount)([[:space:]]|$)' "$td"/D1/formal/fio.job; then die fixture_command; fi
  printf 'T064_DRIVER_SELF_TEST_PASS\tmatrix=%s\n' "$(matrix_order)"
}

case $MODE in
  --self-test|buffered-self-test) offline_self_test_064;;
  plan|buffered-plan) valid_run; write_plans "$PLAN_OUT"; printf 'T064_PLAN_ONLY_PASS\troot=%s\n' "$PLAN_OUT";;
  phase|buffered-phase) phase;;
