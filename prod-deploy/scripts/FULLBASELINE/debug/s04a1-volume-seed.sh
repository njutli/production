...
[[ "$INSTANCE" == SEED-CANARY || "$INSTANCE" == SEED-FORMAL ]] ||
  s04a1_die 'seed script accepts only SEED-CANARY or SEED-FORMAL'
[[ "$CLUSTER" == C ]] || s04a1_die 'seed creation is frozen to cluster C'
...
s04a1_require_tools "$S04A1_JUICEFS_BIN" fio curl python3 sha256sum mountpoint truncate sync
...
verify_mount() {
  [[ -s "$STATE" ]] || s04a1_die 'seed volume state missing'
  mountpoint -q "$MNT" || s04a1_die 'seed mount is not active'
  ...
  [[ -r "/proc/$pid/stat" && $(awk '{print $22}' "/proc/$pid/stat") == "$start" ]] || s04a1_die 'seed mount PID changed'
  [[ $(md5sum "/proc/$pid/exe" | awk '{print $1}') == "$md5" ]] || s04a1_die 'seed mount binary changed'
  pairs=$(mount_pid_pairs); selected=$(s04a1_select_child_pid "$pairs")
  [[ "$selected" == "$pid" ]] || s04a1_die 'seed mount worker changed'
  s04a1_nul_file_has_exact_arg "/proc/$pid/cmdline" "$META" || s04a1_die 'seed mount META changed'
  s04a1_nul_file_has_exact_arg "/proc/$pid/cmdline" "$MNT" || s04a1_die 'seed mount path changed'
  verify_cluster
}

format_mount() {
  [[ ${S04A1_SEED_FORMAT_AUTH:-} == "04-2-seed-format-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    s04a1_die "set exact S04A1_SEED_FORMAT_AUTH=04-2-seed-format-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  s04a1_record_authorization "$RUN_ID" seed-format "$S04A1_SEED_FORMAT_AUTH"
  [[ ! -e "$STATE" && ! -e "$SEED_DIR/SEED_BUNDLE_PASS" && ! -e "$MNT" ]] ||
    s04a1_die 'seed state, bundle, or mount path already exists'
  [[ $(md5sum "$S04A1_JUICEFS_BIN" | awk '{print $1}') == "$S04A1_JUICEFS_MD5" ]] || s04a1_die 'JuiceFS binary MD5 mismatch'
  verify_cluster
  umask 077
  mkdir -p "$OUT" "$SEED_DIR" "$MNT"
  printf '#!/usr/bin/env bash\n# Recorded 04-2 seed commands; no password.\n' > "$OUT/commands.sh"
...
print("%s\t%s\t%s"%(s["objects"],s["stored"],s["bytes_used"]))' > "$SEED_DIR/pool-pre-format.tsv"
  if "$S04A1_JUICEFS_BIN" status "$META" >/dev/null 2>&1; then s04a1_die "seed META already exists: $META"; fi
  record_cmd env "CEPH_CONF=$PRIVATE_CONF" "$S04A1_JUICEFS_BIN" format --storage ceph \
    --bucket "ceph://${S04A1_POOL}" --access-key ceph --secret-key client.juicefs \
    --block-size 256K --trash-days 0 "$META" "$VOLUME"
  "$S04A1_JUICEFS_BIN" format --storage ceph --bucket "ceph://${S04A1_POOL}" \
  ...
  sleep 5
  mountpoint -q "$MNT" || s04a1_die 'seed mount failed'
  local pairs pid identity uuid name
  pairs=$(mount_pid_pairs); pid=$(s04a1_select_child_pid "$pairs")
  "$S04A1_JUICEFS_BIN" status "$META" > "$OUT/status-format.json"
  identity=$(s04a1_status_identity "$OUT/status-format.json") || s04a1_die 'invalid seed identity after format'
  IFS=$'\t' read -r uuid name <<< "$identity"
  [[ "$name" == "$VOLUME" ]] || s04a1_die 'seed volume name mismatch'
  write_mount_state "$pairs" "$pid" "$uuid"
  printf 'SEED_FORMAT_MOUNT_PASS instance=%s uuid=%s pid=%s\n' "$INSTANCE" "$uuid" "$pid"
}

layout_seed() {
  [[ ${S04A1_SEED_LAYOUT_AUTH:-} == "04-2-seed-layout-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] || s04a1_die "set exact S04A1_SEED_LAYOUT_AUTH=04-2-seed-layout-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  s04a1_record_authorization "$RUN_ID" seed-layout "$S04A1_SEED_LAYOUT_AUTH"
  verify_mount
  [[ ! -e "$OUT/LAYOUT_PASS" && ! -e "$SOURCE_DIR" ]] || s04a1_die 'seed layout already exists'
  mkdir -p "$SOURCE_DIR" "$OUT/jobfiles" "$OUT/layout-bw"
  if [[ "$FLAVOR" == formal ]]; then
    bash "$SCRIPT_DIR/s04a1-gen-jobfiles.sh" "$OUT/jobfiles" "$SOURCE_DIR" > "$OUT/jobfiles.log"
    ...
    grep -q 'err= 0' "$OUT/layout-fio.stdout" || s04a1_die 'formal seed layout fio lacks err=0'
    mapfile -t paths < <(awk -F= '$1=="filename"{print substr($0,index($0,"=")+1)}' "$OUT/jobfiles/layout-B0.fio" | sort -u)
    (( ${#paths[@]} == 256 )) || s04a1_die 'formal seed jobfile does not contain 256 unique files'
    for path in "${paths[@]}"; do
      [[ "$path" == "$SOURCE_DIR"/* && -f "$path" && ! -L "$path" ]] || s04a1_die "unsafe seed layout path: $path"
    ...
    grep -q 'err= 0' "$OUT/layout-fio.stdout" || s04a1_die 'canary seed layout fio lacks err=0'
  ...
  [[ $(wc -l < "$OUT/seed-layout-relative.tsv") -eq "$expected" ]] || s04a1_die 'seed layout file count mismatch'
  awk -F '\t' -v s="$size" '$2!=s{bad=1} END{exit bad}' "$OUT/seed-layout-relative.tsv" || s04a1_die 'seed layout size mismatch'
...
umount_seed() {
  [[ ${S04A1_SEED_UMOUNT_AUTH:-} == "04-2-seed-umount-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] || s04a1_die "set exact S04A1_SEED_UMOUNT_AUTH=04-2-seed-umount-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  s04a1_record_authorization "$RUN_ID" seed-umount "$S04A1_SEED_UMOUNT_AUTH"
  verify_mount
  [[ -s "$OUT/LAYOUT_PASS" ]] || s04a1_die 'seed layout gate missing'
  record_cmd "$S04A1_JUICEFS_BIN" umount "$MNT"
  "$S04A1_JUICEFS_BIN" umount "$MNT" > "$OUT/umount.log" 2>&1 || s04a1_die 'graceful seed umount failed'
  mountpoint -q "$MNT" && s04a1_die 'seed mount remains after umount'
...
dump_seed() {
  [[ ${S04A1_SEED_DUMP_AUTH:-} == "04-2-seed-dump-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    s04a1_die "set exact S04A1_SEED_DUMP_AUTH=04-2-seed-dump-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  s04a1_record_authorization "$RUN_ID" seed-dump "$S04A1_SEED_DUMP_AUTH"
  [[ -s "$STATE" && -s "$OUT/LAYOUT_PASS" && -s "$OUT/READY_FOR_FIO" && -s "$OUT/UMOUNT_EPOCH" ]] ||
    s04a1_die 'seed layout/reset/umount evidence incomplete'
  [[ ! -e "$DUMP" && ! -e "$SEED_DIR/SEED_BUNDLE_PASS" ]] || s04a1_die 'seed bundle already exists'
  ! mountpoint -q "$MNT" || s04a1_die 'seed is still mounted'
  local then now identity uuid name
  then=$(<"$OUT/UMOUNT_EPOCH"); now=$(date +%s)
  (( now - then >= 65 )) || s04a1_die "seed session TTL not elapsed; retry after $((65-now+then)) seconds"
  export CEPH_CONF="$PRIVATE_CONF"
  "$S04A1_JUICEFS_BIN" status "$META" > "$OUT/status-pre-dump.json"
  identity=$(s04a1_status_identity "$OUT/status-pre-dump.json") || s04a1_die 'invalid seed identity before dump'
  IFS=$'\t' read -r uuid name <<< "$identity"
  [[ "$uuid" == "$(awk -F '\t' '$1=="uuid"{print $2}' "$STATE")" && "$name" == "$VOLUME" ]] || s04a1_die 'seed UUID/name changed before dump'
  s04a1_status_has_zero_sessions "$OUT/status-pre-dump.json" || s04a1_die 'seed has a live session; refuse dump'
  umask 077
  record_cmd env "CEPH_CONF=$PRIVATE_CONF" "$S04A1_JUICEFS_BIN" dump --keep-secret-key "$META" "$DUMP"
  "$S04A1_JUICEFS_BIN" dump --keep-secret-key "$META" "$DUMP" > "$OUT/dump.stdout" 2> "$OUT/dump.stderr"
  [[ -s "$DUMP" && $(stat -c '%a' "$DUMP") == 600 ]] || s04a1_die 'seed dump missing or not mode 600'
  ...
  JFS_GC_SKIPPEDTIME=0 "$S04A1_JUICEFS_BIN" gc "$META" > "$SEED_DIR/gc-baseline.log" 2>&1
  s04a1_gc_summary "$SEED_DIR/gc-baseline.log" > "$SEED_DIR/gc-baseline.tsv"
  [[ $(awk -F '\t' '$1=="leaked"{print $2}' "$SEED_DIR/gc-baseline.tsv") == 0 ]] || s04a1_die 'seed has leaked objects'
  [[ $(awk -F '\t' '$1=="pending"{print $2}' "$SEED_DIR/gc-baseline.tsv") == 0 ]] || s04a1_die 'seed has pending-delete objects'
  [[ $(awk -F '\t' '$1=="skipped"{print $2}' "$SEED_DIR/gc-baseline.tsv") == 0 ]] || s04a1_die 'seed GC baseline skipped objects'
  sudo ceph df --format=json | python3 -c '
import json,sys
d=json.load(sys.stdin); p=next(x for x in d["pools"] if x["name"]=="juicefs-data"); s=p["stats"]
print("%s\t%s\t%s"%(s["objects"],s["stored"],s["bytes_used"]))' > "$SEED_DIR/pool-seed.tsv"
  printf 'flavor\t%s\nvolume_name\t%s\nuuid\t%s\nsource_dir\t/seed_layout\ndump\t%s\ndump_sha256\t%s\nlayout_sha256\t%s\nanchor_sha256\t%s\njuicefs_md5\t%s\n' \
    ...
    "$(sha256sum "$SEED_DIR/seed-content-anchors.tsv" | awk '{print $1}')" "$S04A1_JUICEFS_MD5" > "$SEED_DIR/seed.tsv"
...
case "$ACTION" in
  ...
  umount) umount_seed;;
  dump) dump_seed;;
  *) s04a1_die 'usage: s04a1-seed-volume.sh format-mount|layout|verify|umount|dump RUN_ID C SEED-CANARY|SEED-FORMAL';;
esac
