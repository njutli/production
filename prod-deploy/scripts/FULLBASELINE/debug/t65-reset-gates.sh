#!/usr/bin/env bash
# Ceph/TiKV stability gates for one 03-22b arm. No cluster or volume lifecycle here.
set -euo pipefail
export LC_ALL=C
...
[[ "$INSTANCE" != SMOKE-* ]] || t65_die 'smoke instances do not run reset gates'
if [[ "$ACTION" == post-abort-final-destroy ]]; then
  [[ ${T65_ABORT_RESET_AUTH:-} == "03-22b-abort-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t65_die "set exact T65_ABORT_RESET_AUTH=03-22b-abort-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  [[ -z ${T65_RESET_AUTH:-} ]] || t65_die 'normal reset authorization is forbidden for abort cleanup'
else
  [[ ${T65_RESET_AUTH:-} == "03-22b-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    t65_die "set exact T65_RESET_AUTH=03-22b-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}"
fi

OUT="/tmp/production/opencode-t3.22b-${RUN_ID}/instances/${INSTANCE}"
if [[ "$ACTION" == post-abort-final-destroy ]]; then
  t65_record_authorization "$RUN_ID" "reset-$ACTION-$INSTANCE" "$T65_ABORT_RESET_AUTH"
else
  t65_record_authorization "$RUN_ID" "reset-$ACTION-$INSTANCE" "$T65_RESET_AUTH"
fi
...
SEED_DIR=''
case "$INSTANCE" in
  SEED-CANARY|SEED-FORMAL|RESTORE-CANARY-A1|RESTORE-CANARY-B1|RESTORE-PREFLIGHT-A1|RESTORE-PREFLIGHT-B1|ARM-CANARY-A1|ARM-CANARY-B1|GC-CANARY|GC-PREFLIGHT|GC-ARM-CANARY|G0[1-8]|R0[1-8])
    FLAVOR=$(t65_seed_flavor "$INSTANCE")
    SEED_DIR=$(t65_seed_dir "$RUN_ID" "$FLAVOR")
    ;;
esac
[[ ! -e "$RESET" ]] || t65_die "reset output already exists; preserve first attempt: $RESET"
mkdir -p "$RESET"
t65_require_tools "$T65_JUICEFS_BIN" python3 curl sudo sshpass ssh
t65_make_ssh_array
export CEPH_CONF="$PRIVATE_CONF"

[[ $(sudo ceph health) == HEALTH_OK ]] || t65_die 'Ceph health is not HEALTH_OK'
! pgrep -x fio >/dev/null || t65_die 'foreign fio exists on client'
for node in "${T65_NODES[@]}"; do
  ! "${T65_SSH[@]}" "$node" 'pgrep -x fio' >/dev/null 2>&1 || t65_die "foreign fio exists on $node"
  mem_kib=$("${T65_SSH[@]}" "$node" "awk '/^MemAvailable:/{print \$2}' /proc/meminfo")
  [[ "$mem_kib" =~ ^[0-9]+$ ]] && (( mem_kib >= 67108864 )) ||
    t65_die "MemAvailable below 64GiB on $node: ${mem_kib:-invalid}KiB"
done
...
pending_total() {
  curl -fsS --connect-timeout 3 --max-time 8 "http://$1:${T65_TIKV_STATUS_PORT}/metrics" |
...
for ((i=0;i<=60;i++)); do
  ...
  for node in "${T65_NODES[@]}"; do
    ...
    t65_nvme_quiet_evidence_ok "$evidence" > "$summary" || return 1
  done
}

mapfile -t OSD_IDS < <(sudo ceph osd ls)
(( ${#OSD_IDS[@]} == 6 )) || t65_die "expected frozen six OSDs, got ${#OSD_IDS[@]}"
printf '%s\n' "${OSD_IDS[@]}" > "$RESET/osd-ids.txt"
sudo ceph osd dump > "$RESET/osd-dump.txt"

case "$ACTION" in
  prepare)
    [[ -s "$OUT/LAYOUT_PASS" && -s "$OUT/volume.tsv" ]] || t65_die 'layout or mounted volume state missing'
    pool_objects > "$RESET/pool-before.tsv"
    compact_and_wait pre-gc || t65_die 'pre-GC OSD cooldown failed'
    printf '%q gc --compact %q\n' "$T65_JUICEFS_BIN" "$META" >> "$OUT/commands.sh"
    "$T65_JUICEFS_BIN" gc --compact "$META" > "$RESET/juicefs-gc.log" 2>&1 || t65_die 'temporary-volume gc --compact failed'
    compact_and_wait post-gc || t65_die 'post-GC OSD cooldown failed'
    tikv_idle_gate || t65_die 'temporary TiKV pending compaction did not reach three consecutive zeros'
    nvme_quiet_gate || t65_die 'underlying NVMe did not meet the frozen bounded-idle profile in the final 30s of a 60s gate'
    [[ $(sudo ceph health) == HEALTH_OK ]] || t65_die 'Ceph changed after quiet period'
    ...
    ;;
  post-destroy)
    [[ -s "$OUT/volume.destroyed.tsv" && ! -e "$OUT/volume.tsv" ]] || t65_die 'volume destroy evidence missing'
    compact_and_wait post-destroy || t65_die 'post-destroy OSD cooldown failed'
...
PY
  ...
  seed-return)
    [[ "$INSTANCE" == GC-CANARY || "$INSTANCE" == GC-PREFLIGHT || "$INSTANCE" == GC-ARM-CANARY || "$INSTANCE" =~ ^G0[1-8]$ ]] ||
      t65_die 'seed-return requires a dedicated GC instance'
    [[ -s "$SEED_DIR/pool-seed.tsv" && -s "$SEED_DIR/gc-baseline.tsv" ]] || t65_die 'seed baselines missing'
    if [[ "$INSTANCE" == GC-PREFLIGHT ]]; then
      [[ -s "$OUT/GC_INSPECT_PASS" && $(awk -F '\t' '$1=="leaked"{print $2}' "$OUT/gc-inspect.tsv") == 0 ]] ||
        t65_die 'GC-PREFLIGHT requires a zero-leak inspect result'
    else
      [[ -s "$OUT/GC_DELETE_PASS" && -s "$OUT/gc-postcheck.tsv" ]] || t65_die 'GC delete/postcheck evidence missing'
    fi
    compact_and_wait seed-return || t65_die 'post-GC OSD cooldown failed'
...
PY
    ...
    ;;
  post-final-destroy)
    [[ "$INSTANCE" == GC-CANARY || "$INSTANCE" == G08 ]] || t65_die 'post-final-destroy requires GC-CANARY or G08'
    [[ -s "$SEED_DIR/seed.destroyed.tsv" && -s "$SEED_DIR/pool-pre-format.tsv" ]] || t65_die 'seed destroy/pre-format evidence missing'
    compact_and_wait final-destroy || t65_die 'post-destroy OSD cooldown failed'
...
PY
  ...
  post-abort-final-destroy)
    [[ "$INSTANCE" =~ ^G0[1-7]$ ]] || t65_die 'post-abort-final-destroy requires an early G01..G07 cleanup instance'
    [[ -s "$OUT/ABORT_SEED_DESTROY_PASS" && -s "$SEED_DIR/seed.destroyed.tsv" &&
      ...
      t65_die 'abort seed destroy evidence or pre-format baseline missing'
    compact_and_wait abort-final-destroy || t65_die 'post-abort-destroy OSD cooldown failed'
...
PY
    ...
    ;;
  *) t65_die 'usage: t65-reset-gates.sh prepare|post-destroy|seed-return|post-final-destroy|post-abort-final-destroy RUN_ID A1|B1 INSTANCE';;
esac
