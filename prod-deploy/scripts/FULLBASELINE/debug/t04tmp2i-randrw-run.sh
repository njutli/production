...
CELL=${3:-}
REQUEST_KIND=R
ROOT=/tmp/production/opencode-04tmp2i-$RUN_ID
die() { printf 'E_04TMP2I\t%s\n' "$*" >&2; exit 42; }
incident() { mkdir -m 0700 -p "$ROOT"; printf '%s\t%s\t%s\n' "$(date +%s%N)" "$1" "$2" >>"$ROOT/incidents.tsv"; }
valid_run() {
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ ]] || die invalid_RUN_ID
  [[ $ROOT == /tmp/production/opencode-04tmp2i-$RUN_ID && $ROOT != / && ! -L $ROOT ]] || die unsafe_result_root
  if [[ -e $ROOT ]]; then
...
ANALYZER=$SCRIPT_DIR/t04tmp2i-randrw-analyze.py
...
cell_spec() {
  ...
  MNT=/tmp/jfs-04tmp2i-$RUN_ID-$CELL-$KIND
...
offline_self_test() {
  RUN_ID=20000101-000000; ROOT=/tmp/production/opencode-04tmp2i-$RUN_ID
  assert_no_forbidden
  [[ $(matrix | wc -l) -eq 5 ]] || die matrix_count
  [[ $(matrix | sed -n '/^T128-R$/p' | wc -l) -eq 1 ]] || die read_matrix
  [[ $(matrix | sed -n '/^T128-W$/p' | wc -l) -eq 1 ]] || die write_matrix
  [[ $(matrix | sed -n '/^T128-P25$/p' | wc -l) -eq 1 ]] || die p25_matrix
  grep -q '^A0-pre$' <(matrix) && grep -q '^A0-post$' <(matrix) || die anchors
  ...
  printf '04TMP2I_OFFLINE_SELF_TEST_PASS mandatory=5 conditional=P50,P75 capacity=128\n'
}
make_ceph_conf() {
...
PY
  ...
  if pgrep -a -f '/tmp/jfs-04tmp2i-[0-9]{8}-[0-9]{6}-' >"$ROOT/inventory/tmp2i-processes.tsv" 2>&1; then
    die stale_tmp2i_process
  fi
...
sudo install -d -m 0700 -o 1002 -g 1002 /mnt/jfs-cache/jfs-04tmp2i-$RUN_ID
sudo losetup --find --show --nooverlap /mnt/jfs-cache/jfs-04tmp2i-$RUN_ID/CELL.img
sudo mkfs.ext4 -F -m 0 -E nodiscard,lazy_itable_init=0,lazy_journal_init=0 LOOP_DEVICE
sudo mount -o noatime,nodiscard LOOP_DEVICE /tmp/jfs-04tmp2i-cache-$RUN_ID-CELL
sudo chown 1002:1002 /tmp/jfs-04tmp2i-cache-$RUN_ID-CELL
sudo umount /tmp/jfs-04tmp2i-cache-$RUN_ID-CELL
sudo losetup -d LOOP_DEVICE
sudo rmdir /mnt/jfs-cache/jfs-04tmp2i-$RUN_ID
EOF
  local osd
  while read -r osd; do
...
mount_jfs() {
  local tag=$1
  local -a cmd
  [[ $MNT == /tmp/jfs-04tmp2i-$RUN_ID-$CELL-$KIND && $MNT != / && ! -L $MNT ]] || die unsafe_mount_path
...
runtime_sampler() {
  ...
  printf 'epoch_ns\thit_bytes\tmiss_bytes\tblockcache_bytes\tstaging_blocks\tstaging_bytes\tstaging_writing_blocks\tavailable_bytes\tevicts\tdrops\n' >"$out"
...
PY
    ...
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$epoch_ns" "$hit" "$miss" "$cachebytes" "$rawb" "$rawbytes" "$writing" "$free" "$evict" "$drop" >>"$out"
...
prepare_cache() {
  [[ $KIND != A0 ]] || return 0
  BACKING_ROOT=/mnt/jfs-cache/jfs-04tmp2i-$RUN_ID
  CACHE_MNT=/tmp/jfs-04tmp2i-cache-$RUN_ID-$CELL
  BACKING=$BACKING_ROOT/$CELL.img
  CACHE_DIR=$CACHE_MNT/cache
  local parent_avail
  [[ $BACKING_ROOT == /mnt/jfs-cache/jfs-04tmp2i-$RUN_ID &&
     $CACHE_MNT == /tmp/jfs-04tmp2i-cache-$RUN_ID-$CELL &&
     $BACKING == /mnt/jfs-cache/jfs-04tmp2i-$RUN_ID/$CELL.img &&
...
validate_p25_sampling() {
  python3 "$ANALYZER" validate-cell --root "$ROOT" --name T128-P25 \
    --output "$ROOT/cells/T128-P25/sampling-validation.json" >/dev/null || die p25_sampling_canary_failed
}
p25_supplement_decision() {
  python3 - "$ROOT/summary.tsv" <<'PY'
import csv,sys
rows={row['cell']:(float(row['read_mib_s']),float(row['write_mib_s']),float(row['mean_direction_mib_s']))
      for row in csv.DictReader(open(sys.argv[1]),delimiter='\t')}
if not {'A0-pre','T128-P25'} <= set(rows):
    raise SystemExit('P25 provisional inputs missing')
anchor,point=rows['A0-pre'],rows['T128-P25']
effect=[point[i]/anchor[i]-1 for i in range(3)]
print('ADD' if effect[2] >= 0 and effect[0] > -0.05 and effect[1] > -0.05 else 'SKIP')
PY
}
execute_all() {
  valid_run
  [[ ${TMP2I_ACK:-} == I_ACK_04TMP2I_$RUN_ID ]] || die ack_missing
  ...
  sha256sum "$0" "$ANALYZER" "$SCRIPT_DIR/t04tmp2i-randrw-gate0-offline.sh" "$SCRUB" >"$ROOT/runtime-scripts.sha256"
  trap on_exit EXIT
  pause_scrub
  ...
  run_token T128-P25
  validate_p25_sampling
  supplement=$(p25_supplement_decision)
  printf 'T128\t%s\n' "$supplement" >"$ROOT/supplement-decisions.tsv"
  if [[ $supplement == ADD ]]; then
    run_token T128-P50
    run_token T128-P75
  fi
  run_token T128-R
...
resume_postfio() {
  ...
  [[ $BACKING == /mnt/jfs-cache/jfs-04tmp2i-$RUN_ID/$CELL.img && $CACHE_MNT == /tmp/jfs-04tmp2i-cache-$RUN_ID-$CELL ]] || die resume_storage_path
  static_identity
...
bundle_run() {
  ...
  local tarball=$ROOT/bundle/04-tmp2i-$RUN_ID.tar
  ...
  tar --sort=name --mtime='UTC 1970-01-01' -cf "$tarball" -C "$ROOT" "${files[@]}" \
    -C "$SCRIPT_DIR" t04tmp2i-randrw-run.sh t04tmp2i-randrw-analyze.py \
    t04tmp2i-randrw-gate0-offline.sh u141d-scrub-control.sh 2>/dev/null || die bundle_failed
