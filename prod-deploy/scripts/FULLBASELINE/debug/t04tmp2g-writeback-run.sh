...
ROOT=/tmp/production/opencode-04tmp2g-$RUN_ID
...
BACKING_ROOT=$CACHE_PARENT/jfs-04tmp2g-$RUN_ID
...
die(){ printf 'E_04TMP2G\t%s\n' "$*" >&2; exit 42; }
valid_run(){
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ ]] || die invalid_RUN_ID
  [[ $ROOT == "/tmp/production/opencode-04tmp2g-$RUN_ID" && $ROOT != / && $ROOT != *..* ]] || die unsafe_result_root
  [[ $BACKING_ROOT == "/mnt/jfs-cache/jfs-04tmp2g-$RUN_ID" && $BACKING_ROOT != / && $BACKING_ROOT != *..* ]] || die unsafe_backing_root
  [[ ! -L $ROOT && ! -L $BACKING_ROOT ]] || die symlink_root
}
cell_fields(){
  case $CELL in
    W20A-randwrite|W20B-randwrite) BACKING_GIB=20;;
  ...
  JOBS=128; FILE_BYTES=1073741824; JFS_MNT=/tmp/jfs-04tmp2g-mnt-$RUN_ID-$CELL
  CELL_ROOT=$ROOT/cells/$CELL; STATE=$CELL_ROOT/state.tsv
  WB=1; TIER_MIB=1; BACKING=$BACKING_ROOT/$CELL.img
  CACHE_MNT=/tmp/jfs-04tmp2g-cache-$RUN_ID-$CELL; CACHE_DIR=$CACHE_MNT/cache
}
valid_paths(){
  cell_fields
  [[ $JFS_MNT == "/tmp/jfs-04tmp2g-mnt-$RUN_ID-$CELL" && $CELL_ROOT == "$ROOT/cells/$CELL" ]] || die unsafe_cell_path
...
write_fio(){
  ...
  ! grep -Eq '^(time_based|runtime)=' "$out/randwrite.fio" || die time_contract_drift
  printf '137438953472\n' >"$out/fixed-total-write-bytes.txt"
  printf '%s\n' "$(date +%s%N)" >"$out/fio-start-epoch-ns.txt"
  local rc
  if timeout 900 fio "$out/randwrite.fio" --output-format=json --output="$out/fio.json" --write_bw_log="$out/bw/randwrite" --log_avg_msec=1000 --per_job_logs=1; then rc=0; else rc=$?; fi
...
sizes=[int(job.get('write',{}).get('io_bytes',0)) for job in jobs]
if any(x != 1073741824 for x in sizes) or sum(sizes) != 137438953472:
    raise SystemExit(f'fixed write contract failed: total={sum(sizes)}')
...
matrix_guard(){
  ...
  case ${#names[@]} in
    0) [[ $CELL == W20A-randwrite ]] || die matrix_expected_W20A;;
    1) [[ ${names[0]} == W20A-randwrite && $CELL == W32-randwrite ]] || die matrix_expected_W32;;
    2) [[ ${names[0]} == W20A-randwrite && ${names[1]} == W32-randwrite && $CELL == W64-randwrite ]] || die matrix_expected_W64;;
    3) [[ ${names[2]} == W64-randwrite && $CELL == W128-randwrite ]] || die matrix_expected_W128;;
    4) [[ ${names[3]} == W128-randwrite && $CELL == W20B-randwrite ]] || die matrix_expected_W20B;;
...
PY
  ...
  printf '04TMP2G_INVENTORY_PASS root=%s\n' "$ROOT"
}
cmd_plan(){
...
For each CELL in W20A/W32/W64/W128/W20B-randwrite, one at a time:
sudo install -d -m 0700 -o 1002 -g 1002 $BACKING_ROOT
fallocate -l <20G|32G|64G|128G> -- $BACKING_ROOT/<CELL>.img
...
sudo mount -o noatime,nodiscard <same-loop> /tmp/jfs-04tmp2g-cache-$RUN_ID-<CELL>
sudo chown 1002:1002 /tmp/jfs-04tmp2g-cache-$RUN_ID-<CELL>
sudo umount /tmp/jfs-04tmp2g-cache-$RUN_ID-<CELL>
...
TMP2G_RECOVERY_ACK=I_ACK_04TMP2G_RECOVERY_$RUN_ID bash $SCRIPT_DIR/t04tmp2g-writeback-recovery.sh run $RUN_ID after-<CELL>
...
EOF
  printf 'W20A-randwrite\nW32-randwrite\nW64-randwrite\nW128-randwrite\nW20B-randwrite\n' >"$ROOT/plans/matrix-order.txt"
  printf 'cell\tverdict\tepoch\n' >"$ROOT/run-state.tsv"
  sha256sum "$SCRIPT_DIR/t04tmp2g-writeback-run.sh" "$SCRIPT_DIR/t04tmp2g-writeback-analyze.py" \
    "$SCRIPT_DIR/t04tmp2g-writeback-recovery.sh" \
    "$SCRIPT_DIR/t04tmp2g-writeback-gate0-offline.sh" \
    "$SCRIPT_DIR/u141d-scrub-control.sh" >"$ROOT/plans/scripts.sha256"
  printf 'PLAN_PASS\n' >"$ROOT/plans/PASS"; printf '04TMP2G_PLAN_PASS root=%s\n' "$ROOT"; }
cmd_pause_scrub(){
  valid_run; [[ -f $ROOT/plans/PASS && -r $ROOT/inventory/ceph-fsid.txt ]] || die plan_missing
  [[ ${TMP2G_SCRUB_ACK:-} == I_ACK_GLOBAL_CEPH_SCRUB_PAUSE ]] || die scrub_ack_missing
...
cmd_restore_scrub(){
  ...
  printf '04TMP2G_SCRUB_RESTORE_PASS run=%s\n' "$RUN_ID"
}
cmd_run(){
  valid_run; valid_paths
  [[ ${TMP2G_ACK:-} == I_ACK_04TMP2G_$RUN_ID ]] || die ack_missing
  ...
  printf '04TMP2G_CELL_COMPLETE cell=%s verdict=%s\n' "$CELL" "$verdict"
  printf 'NEXT_REQUIRED_RECOVERY: TMP2G_RECOVERY_ACK=I_ACK_04TMP2G_RECOVERY_%s bash %q run %q %q\n' \
    "$RUN_ID" "$SCRIPT_DIR/t04tmp2g-writeback-recovery.sh" "$RUN_ID" "after-$CELL"
...
sizes=[int(j.get('write',{}).get('io_bytes',0)) for j in jobs]
assert all(x==1073741824 for x in sizes) and sum(sizes)==137438953472
PY
...
PY
  ...
  *) die invalid_resume_drain_status;;
  ...
  printf 'CELL_COMPLETE\t%s\t%s\n' "$CELL" "$verdict" >"$CELL_ROOT/PASS"
  printf '%s\t%s\t%s\n' "$CELL" "$verdict" "$(date +%s)" >>"$ROOT/run-state.tsv"
  printf '04TMP2G_RESUME_POSTFIO_COMPLETE cell=%s verdict=%s\n' "$CELL" "$verdict"
  printf 'NEXT_REQUIRED_RECOVERY: TMP2G_RECOVERY_ACK=I_ACK_04TMP2G_RECOVERY_%s bash %q run %q %q\n' \
    "$RUN_ID" "$SCRIPT_DIR/t04tmp2g-writeback-recovery.sh" "$RUN_ID" "after-$CELL"
  [[ $verdict != INVALID_AND_STOP ]] || die noncapacity_upload_error
}
cmd_offline(){
  valid_run
  local candidate
  for candidate in W20A-randwrite W32-randwrite W64-randwrite W128-randwrite W20B-randwrite; do
    CELL=$candidate; valid_paths
  done
  ! grep -Eq 'mkfs\.ext4[^\n]*-T[[:space:]]+largefile' "$0" || die largefile_forbidden
  grep -q -- '--writeback' "$0"
  printf '04TMP2G_OFFLINE_SELF_TEST_PASS cells=5 fixed_write_gib=128 direct_backing_gib=20,32,64,128,20\n'
}
