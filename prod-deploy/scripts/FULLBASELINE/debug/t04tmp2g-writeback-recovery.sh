...
die(){ printf 'E_04TMP2G_RECOVERY\t%s\n' "$*" >&2; exit 42; }
valid(){
  [[ $RUN_ID =~ ^[0-9]{8}-[0-9]{6}$ ]] || die invalid_RUN_ID
  [[ $LABEL =~ ^after-W(20A|20B|32|64|128)-randwrite$ ]] || die invalid_label
  [[ $ROOT == /tmp/production/opencode-04tmp2g-$RUN_ID && $ROOT != / && $ROOT != *..* ]] || die unsafe_root
...
health(){
  local file=$1
  env U141D_SCRUB_STATE_DIR="$ROOT" U141D_CEPH_CONF="$CEPH_CONF" bash "$SCRUB" verify-paused "$LEASE" >"${file%.json}-scrub-lease.txt" || die scrub_lease_invalid
...
if not (status=='HEALTH_OK' and not keys or status=='HEALTH_WARN' and keys=={'OSDMAP_FLAGS'}):
    raise SystemExit(f'unexpected health status={status} checks={sorted(keys)}')
...
cmd_run(){
  valid
  [[ ${TMP2G_RECOVERY_ACK:-} == I_ACK_04TMP2G_RECOVERY_$RUN_ID ]] || die ack_missing
  ...
  printf '04TMP2G_RECOVERY_PASS label=%s objects=%s seed=%s\n' "$LABEL" "$after" "$seed"
}
cmd_offline(){
  RUN_ID=20260905-141236; LABEL=after-W20A-randwrite
  ROOT=/tmp/production/opencode-04tmp2g-$RUN_ID; CEPH_CONF=$ROOT/inventory/ceph-msgr8.conf; OUT=$ROOT/recovery/$LABEL
  [[ $ROOT != / && $OUT == "$ROOT/recovery/after-W20A-randwrite" ]] || die offline_scope
  printf '04TMP2G_RECOVERY_OFFLINE_SELF_TEST_PASS\n'
}
