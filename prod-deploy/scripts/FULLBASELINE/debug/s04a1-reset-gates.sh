#!/usr/bin/env bash
# Ceph/TiKV stability gates for one 04-2 arm. No cluster or volume lifecycle here.
set -euo pipefail
export LC_ALL=C
...
[[ "$INSTANCE" != SMOKE-* ]] || s04a1_die 'smoke instances do not run reset gates'
if [[ "$ACTION" == post-abort-final-destroy ]]; then
  [[ ${S04A1_ABORT_RESET_AUTH:-} == "04-2-abort-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    s04a1_die "set exact S04A1_ABORT_RESET_AUTH=04-2-abort-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}"
  [[ -z ${S04A1_RESET_AUTH:-} ]] || s04a1_die 'normal reset authorization is forbidden for abort cleanup'
else
  [[ ${S04A1_RESET_AUTH:-} == "04-2-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}" ]] ||
    s04a1_die "set exact S04A1_RESET_AUTH=04-2-reset-${RUN_ID}-${INSTANCE}-${CLUSTER}"
fi

OUT="/tmp/production/opencode-04-2-${RUN_ID}/instances/${INSTANCE}"
if [[ "$ACTION" == post-abort-final-destroy ]]; then
  s04a1_record_authorization "$RUN_ID" "reset-$ACTION-$INSTANCE" "$S04A1_ABORT_RESET_AUTH"
else
  s04a1_record_authorization "$RUN_ID" "reset-$ACTION-$INSTANCE" "$S04A1_RESET_AUTH"
fi
...
SEED_DIR=''
case "$INSTANCE" in
  SEED-CANARY|SEED-FORMAL|RESTORE-CANARY-C|RESTORE-CANARY-L|RESTORE-PREFLIGHT-C|RESTORE-PREFLIGHT-L|ARM-CANARY-C|ARM-CANARY-L|GC-CANARY|GC-PREFLIGHT|GC-ARM-CANARY|GC-ARM-CANARY-C|GC-ARM-CANARY-L|G0[1-8]|R0[1-8])
    FLAVOR=$(s04a1_seed_flavor "$INSTANCE")
    SEED_DIR=$(s04a1_seed_dir "$RUN_ID" "$FLAVOR")
    ;;
esac
[[ ! -e "$RESET" ]] || s04a1_die "reset output already exists; preserve first attempt: $RESET"
mkdir -p "$RESET"
s04a1_require_tools "$S04A1_JUICEFS_BIN" python3 curl sudo ssh
...
for node in "${S04A1_NODES[@]}"; do
  ! "${S04A1_SSH[@]}" "$node" 'pgrep -x fio' >/dev/null 2>&1 || s04a1_die "foreign fio exists on $node"
  mem_kib=$("${S04A1_SSH[@]}" "$node" "awk '/^MemAvailable:/{print \$2}' /proc/meminfo")
  [[ "$mem_kib" =~ ^[0-9]+$ ]] && (( mem_kib >= 67108864 )) ||
    s04a1_die "MemAvailable below 64GiB on $node: ${mem_kib:-invalid}KiB"
done
...
pending_total() {
  curl -fsS --connect-timeout 3 --max-time 8 "http://$1:${S04A1_TIKV_STATUS_PORT}/metrics" |
...
nvme_quiet_gate() {
  ...
  declare -a pids=()
  for node in "${S04A1_NODES[@]}"; do
    "${S04A1_SSH[@]}" "$node" 'set -euo pipefail
...
for ((i=0;i<=60;i++)); do
  ...
  for node in "${S04A1_NODES[@]}"; do
    ...
    s04a1_nvme_quiet_evidence_ok "$evidence" > "$summary" || return 1
  done
}

mapfile -t OSD_IDS < <(sudo ceph osd ls)
(( ${#OSD_IDS[@]} == 6 )) || s04a1_die "expected frozen six OSDs, got ${#OSD_IDS[@]}"
printf '%s\n' "${OSD_IDS[@]}" > "$RESET/osd-ids.txt"
sudo ceph osd dump > "$RESET/osd-dump.txt"

case "$ACTION" in
  prepare)
    [[ -s "$OUT/LAYOUT_PASS" && -s "$OUT/volume.tsv" ]] || s04a1_die 'layout or mounted volume state missing'
    pool_objects > "$RESET/pool-before.tsv"
    # One fixed Ceph compaction point per reset is the approved maximum.
    # The restored clone is fresh and has no foreground writes yet, so a
    # second JuiceFS/OSD compaction would add time without improving control.
    compact_and_wait pre-arm || s04a1_die 'pre-arm OSD cooldown failed'
    tikv_idle_gate || s04a1_die 'temporary TiKV pending compaction did not reach three consecutive zeros'
    nvme_quiet_gate || s04a1_die 'underlying NVMe did not meet the frozen bounded-idle profile in the final 30s of a 60s gate'
    s04a1_ceph_health_test_ok || s04a1_die 'Ceph changed outside the approved test/scrub state after quiet period'
    ...
    ;;
  post-destroy)
    [[ -s "$OUT/volume.destroyed.tsv" && ! -e "$OUT/volume.tsv" ]] || s04a1_die 'volume destroy evidence missing'
    compact_and_wait post-destroy || s04a1_die 'post-destroy OSD cooldown failed'
...
PY
    ...
    ;;
  seed-return)
    [[ "$INSTANCE" == GC-CANARY || "$INSTANCE" == GC-PREFLIGHT || "$INSTANCE" == GC-ARM-CANARY || "$INSTANCE" == GC-ARM-CANARY-C || "$INSTANCE" == GC-ARM-CANARY-L || "$INSTANCE" =~ ^G0[1-8]$ ]] ||
      s04a1_die 'seed-return requires a dedicated GC instance'
    [[ -s "$SEED_DIR/pool-seed.tsv" && -s "$SEED_DIR/gc-baseline.tsv" ]] || s04a1_die 'seed baselines missing'
    if [[ "$INSTANCE" == GC-PREFLIGHT ]]; then
      [[ -s "$OUT/GC_INSPECT_PASS" && $(awk -F '\t' '$1=="leaked"{print $2}' "$OUT/gc-inspect.tsv") == 0 ]] ||
        s04a1_die 'GC-PREFLIGHT requires a zero-leak inspect result'
    else
      [[ -s "$OUT/GC_DELETE_PASS" && -s "$OUT/gc-postcheck.tsv" ]] || s04a1_die 'GC delete/postcheck evidence missing'
    fi
    compact_and_wait seed-return || s04a1_die 'post-GC OSD cooldown failed'
...
PY
    ...
    ;;
  post-final-destroy)
    [[ "$INSTANCE" == GC-CANARY || "$INSTANCE" == G08 ]] || s04a1_die 'post-final-destroy requires GC-CANARY or G08'
    [[ -s "$SEED_DIR/seed.destroyed.tsv" && -s "$SEED_DIR/pool-pre-format.tsv" ]] || s04a1_die 'seed destroy/pre-format evidence missing'
    compact_and_wait final-destroy || s04a1_die 'post-destroy OSD cooldown failed'
...
PY
  ...
  post-abort-final-destroy)
    [[ "$INSTANCE" =~ ^G0[1-7]$ ]] || s04a1_die 'post-abort-final-destroy requires an early G01..G07 cleanup instance'
    [[ -s "$OUT/ABORT_SEED_DESTROY_PASS" && -s "$SEED_DIR/seed.destroyed.tsv" &&
      ...
      s04a1_die 'abort seed destroy evidence or pre-format baseline missing'
    compact_and_wait abort-final-destroy || s04a1_die 'post-abort-destroy OSD cooldown failed'
...
PY
    ...
    ;;
  *) s04a1_die 'usage: s04a1-reset-gates.sh prepare|post-destroy|seed-return|post-final-destroy|post-abort-final-destroy RUN_ID C|L INSTANCE';;
esac
