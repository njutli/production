...
OUT=${1:-}
TEST_DIR=${2:-}
[[ "$OUT" == /tmp/production/* && "$TEST_DIR" =~ ^/tmp/jfs-t65-[0-9]{8}-[0-9]{6}-mnt-(R0[1-8]|ARM-CANARY-(A1|B1)|SEED-FORMAL|RESTORE-(CANARY|PREFLIGHT)-(A1|B1))/(test_dir|seed_layout)$ ]] || {
  printf 'E_T65\tunsafe output or test directory\n' >&2
  exit 42
}
mkdir -p "$OUT"
