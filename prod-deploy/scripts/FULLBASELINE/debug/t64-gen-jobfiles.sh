...
OUT=${1:-}
TEST_DIR=${2:-}
[[ "$OUT" == /tmp/production/* && "$TEST_DIR" =~ ^/tmp/jfs-t64-[0-9]{8}-[0-9]{6}-mnt-(R0[1-8]|LAYOUT-CANARY-A|ARM-CANARY-A2?|SEED-FORMAL|RESTORE-(CANARY|PREFLIGHT))/(test_dir|seed_layout)$ ]] || {
  printf 'E_T64\tunsafe output or test directory\n' >&2
  exit 42
}
mkdir -p "$OUT"
