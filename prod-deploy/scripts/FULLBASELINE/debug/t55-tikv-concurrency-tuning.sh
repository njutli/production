...
META='tikv://10.20.1.150:2379,10.20.1.151:2379,10.20.1.152:2379/juicefs-prod?max-tso-batch-wait-interval=1ms&open-tso-follower-proxy=true'
MNT=/mnt/juicefs
TEST_DIR=/mnt/juicefs/test_dir
MOUNT_OPTS='--max-uploads 300 --cache-size 0 --max-fuse-io 256K'
...
flock -n 9 || { echo "STOP: another T55 batch holds /tmp/t54-stage03.lock"; exit 3; }
...
start_samplers() {
  ...
  bash "$INSTR" start "$OUT/client" T55
  instr_started=1
...
stop_samplers() {
  ...
  if [[ "$instr_started" == 1 ]]; then
    bash "$INSTR" stop "$OUT/client" T55 || true
    instr_started=0
  fi
}
...
on_exit() {
  ...
  trap - EXIT INT TERM
  stop_samplers || true
  if [[ "$rc" -ne 0 ]]; then log "T55 exit rc=$rc；保留全部现场，不自动清理/重跑"; fi
  exit "$rc"
}
trap on_exit EXIT
trap 'terminate_load "wrapper received signal"; exit 130' INT TERM

log "=== T55 03-18b start run_id=$RUN_ID ==="
safety_dir "$OUT"
...

{
  ...
  echo '# T55 actual command record'
  printf 'export PATH=%q:$PATH\n' "$BIN_DIR"
  printf 'export CEPH_CONF=%q\n' "$PRIVATE_CONF"
  printf '%q mount -d %s %q %q\n' "$BIN" "$MOUNT_OPTS" "$META" "$MNT"
  printf 'ITEMS=randwrite SKIP_REMOUNT=1 OBJ_GATE=1 OBJ_GC_PASSES=0 OBJ_START_MAX=%s OBJ_WARN=%s OBJ_MAX=%s JUICEFS_MOUNT_OPTS=%q bash %q T55 180 3\n' \
...
setsid env \
  ...
  bash "$V4" T55 180 3 > "$OUT/v4.stdout.log" 2>&1 &
load_pid=$!
...
while kill -0 "$load_pid" 2>/dev/null; do
  ...
  gate_file="$V4_RESULTS/obj-gate-T55.tsv"
...
log "T55 data collection complete; preparing immutable manifest/archive"
log "OUT=$OUT"
log "ARCHIVE=$archive"
