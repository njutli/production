#!/bin/bash
# U141d-only derivative of frozen FULLBASELINE_V4.sh.
# Base MD5: 4198ea2676ba56744a3cd5eba17a5eab.  The benchmark body, item order,
# fio parameters and evidence logic are unchanged; the sole functional extension
# is an opt-in, exact HEALTH_WARN/OSDMAP_FLAGS gate for a verified scrub lease.
set -euo pipefail
export LC_ALL=C
...
# 条件性测试基线：默认 0，保持历史 HEALTH_OK/clock-skew 行为；仅由已完成
# 独立 scrub lease 验证的任务显式设为 1。开启时仍只接受 OSDMAP_FLAGS 唯一告警，
# 且必须同时存在 noscrub + nodeep-scrub、所有 OSD up/in。
CEPH_SCRUB_CONTROLLED="${CEPH_SCRUB_CONTROLLED:-0}"
...
validate_controlled_scrub_health() {
    local health_json="${RESULTS}/ceph-health-pre-${LABEL}.json"
    local osd_json="${RESULTS}/ceph-osd-dump-pre-${LABEL}.json"
    sudo ceph health detail --format json > "${health_json}" 2>/dev/null || return 1
    sudo ceph osd dump --format json > "${osd_json}" 2>/dev/null || return 1
    python3 - "${health_json}" "${osd_json}" <<'PY'
import json, sys
...
keys = sorted(str(k) for k in checks)
if not ((status == "HEALTH_OK" and not keys) or
        (status == "HEALTH_WARN" and keys == ["OSDMAP_FLAGS"])):
    raise SystemExit("unexpected health status/checks: %s/%s" % (status, keys))
raw = osd_dump.get("flags", [])
if isinstance(raw, str):
    flags = [x for x in raw.split(",") if x]
elif isinstance(raw, list):
    flags = [str(x) for x in raw]
else:
    raise SystemExit("unsupported flags type")
flags = {x.strip().replace("nodeep_scrub", "nodeep-scrub") for x in flags if x.strip()}
missing = {"noscrub", "nodeep-scrub"} - flags
if missing:
    raise SystemExit("controlled scrub flags missing: %s" % sorted(missing))
osds = osd_dump.get("osds")
if not isinstance(osds, list) or not osds:
    raise SystemExit("OSD dump has no osds")
if any(x.get("up") != 1 or x.get("in") != 1 for x in osds):
    raise SystemExit("not all OSDs are up/in")
print("CONTROLLED_SCRUB_HEALTH_PASS status=%s checks=%s flags=%s osds=%d" %
      (status, ",".join(keys) or "none", ",".join(sorted(flags)), len(osds)))
PY
}
...
HEALTH=$(sudo ceph health 2>/dev/null || echo "ERROR")
log "ceph health: ${HEALTH}"
if [ "${CEPH_SCRUB_CONTROLLED}" = "1" ]; then
    validate_controlled_scrub_health \
        || { log "ERROR: controlled scrub health contract failed"; exit 1; }
    log "  SCRUB_PAUSED_FOR_CONTROLLED_BENCHMARK: exact health/flags gate PASS"
elif [ "${HEALTH}" = "HEALTH_OK" ]; then
    :
elif echo "${HEALTH}" | grep -q "clock skew"; then
    log "  ⚠️ clock skew，继续"
else
    log "ERROR: health 非 OK"
    exit 1
fi
OSD_UP=$(sudo ceph osd stat 2>/dev/null | grep -oE '[0-9]+ up' | grep -oE '^[0-9]+' || echo 0)
log "OSDs up: ${OSD_UP} (期望 6)"
