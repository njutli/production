...
> ⚠️ **2026-06 更新**：本文核心前提（node1/node2 网卡 100Mb/s）**已失效**。
> 网络已全部升级到千兆，详见 `03-env-change-2026-06.md`。
> "第一优先级：修复网络"已完成，后续重点转向多 RGW 与 EC→3 副本。
> 下文保留作为升级前的分析记录与方法论参考。
>
...
> ❌ **已排除（2026-06）**：存储规格硬性要求 EC 4+2 且空间利用率 ≥60%
> （EC 4+2 = 66.7%，多副本 size=3 仅 33%）。本方案不再考虑，
> 优化集中在网络（已完成）与多 RGW。详见 `03-env-change-2026-06.md`。
